...
W tym artykule założono, że znasz już język programowania. W przeciwnym razie zalecamy przeszukanie jednego z przewodników Szybki Start, takich jak tworzenie aplikacji sieci Web przy użyciu języka [Python](../ide/quickstart-python.md) lub [C#](../get-started/csharp/tutorial-aspnet-core.md)Tworzenie aplikacji konsolowej przy użyciu [Visual Basic](../ide/quickstart-visual-basic-console.md) lub [C++](/cpp/get-started/tutorial-console-cpp).
...
1. Umieść kursor tuż nad ostatnim zamykającym nawiasem klamrowym **w pliku** , a następnie wpisz znaki `svm`. (`svm` oznacza `static void Main`; Metoda [Main ()](/dotnet/csharp/programming-guide/main-and-command-args/) jest punktem wejścia dla C# aplikacji.
...
1. Naciśnij dwukrotnie klawisz **Tab** , aby wstawić fragment kodu.
   ...
   Zostanie wyświetlony podpis metody `static void Main()` dodany do pliku.

Dostępne fragmenty kodu są różne dla różnych języków programowania. Aby sprawdzić dostępne fragmenty kodu dla danego języka, wybierz opcję **edytuj**  > **IntelliSense**  > **Wstaw**wstawki, a następnie wybierz folder języka. W C#przypadku, lista wygląda następująco:
