...
Zapotrzebowanie na łącza sprawdza uprawnienia tylko bezpośredniego wywołującego. Jeśli członek `X` nie ma żadnych żądań związanych z zabezpieczeniami dla jego obiektów wywołujących i wywołuje kod chroniony przez żądanie linku, obiekt wywołujący bez wystarczających uprawnień może użyć `X`, aby uzyskać dostęp do chronionego elementu członkowskiego.
...
W poniższych przykładach przedstawiono bibliotekę, która narusza regułę, oraz aplikację, która demonstruje słabość biblioteki. Biblioteka Przykładowa zawiera dwie metody, które wspólnie naruszają daną regułę. Metoda `EnvironmentSetting` jest zabezpieczona przez żądanie linku dla nieograniczonego dostępu do zmiennych środowiskowych. Metoda `DomainInformation` nie wykonuje żadnych żądań związanych z zabezpieczeniami przed wywołaniem `EnvironmentSetting`.
