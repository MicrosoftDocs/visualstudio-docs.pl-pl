...
|Adnotacja|Opis|
|----------------|-----------------|
|`_Called_from_function_class_(name)`|Nie przeznaczony do autonomicznego; Zamiast tego jest predykatem, który ma być używany z adnotacją `_When_`. Aby uzyskać więcej informacji, zobacz [Określanie, kiedy i gdzie ma zastosowanie adnotacja](../code-quality/specifying-when-and-where-an-annotation-applies.md).<br /><br /> Parametr `name` jest dowolnym ciągiem, który również występuje w adnotacji `_Function_class_` w deklaracji niektórych funkcji.  `_Called_from_function_class_` zwraca wartość różną od zera, jeśli funkcja, która jest obecnie analizowana, ma adnotację przy użyciu `_Function_class_` o tej samej wartości `name`; w przeciwnym razie zwraca wartość zero.|
...
|`_Function_class_(name)`|Parametr `name` jest dowolnym ciągiem, który jest wyznaczany przez użytkownika.  Istnieje w przestrzeni nazw, która różni się od innych przestrzeni nazw. Funkcja, wskaźnik funkcji, lub — najbardziej użyteczny — typ wskaźnika funkcji może być wyznaczono jako należące do jednej lub kilku klas funkcji.|
|`_Raises_SEH_exception_`|Adnotuj funkcję, która zawsze zgłasza wyjątek strukturalny programu obsługi wyjątków (SEH), z zastrzeżeniem `_When_` i `_On_failure_`. Aby uzyskać więcej informacji, zobacz [Określanie, kiedy i gdzie ma zastosowanie adnotacja](../code-quality/specifying-when-and-where-an-annotation-applies.md).|
|`_Maybe_raises_SEH_exception_`|Adnotuj funkcję, która może opcjonalnie podnieść wyjątek SEH, z zastrzeżeniem `_When_` i `_On_failure_`.|
...
|`_Use_decl_annotations_`|Może być używany w definicji funkcji (znanej również jako treść funkcji) zamiast listy adnotacji w nagłówku.  Gdy zostanie użyta `_Use_decl_annotations_`, adnotacje, które pojawiają się w nagłówku w zakresie dla tej samej funkcji, są używane tak, jakby znajdowały się również w definicji zawierającej adnotację `_Use_decl_annotations_`.|

## <a name="successfailure-annotations"></a>Adnotacje sukcesu/niepowodzeń
Funkcja może zakończyć się niepowodzeniem, a gdy tak się stanie, jego wyniki mogą być niekompletne lub inne niż wyniki, gdy funkcja się powiedzie.  Adnotacje na poniższej liście zawierają sposoby wyznaczania zachowania awarii.  Aby użyć tych adnotacji, należy je włączyć, aby określić sukces; w związku z tym wymagana jest adnotacja `_Success_`.  Zwróć uwagę, że `NTSTATUS` i `HRESULT` mają już wbudowaną adnotację `_Success_`; Jednak w przypadku określenia własnej adnotacji `_Success_` na `NTSTATUS` lub `HRESULT` zastępuje wbudowaną adnotację.

|Adnotacja|Opis|
|----------------|-----------------|
|`_Always_(anno_list)`|Równoważne `anno_list _On_failure_(anno_list)`; oznacza to, że adnotacje w `anno_list` mają zastosowanie, niezależnie od tego, czy funkcja się powiedzie.|
|`_On_failure_(anno_list)`|Do użycia tylko wtedy, gdy `_Success_` jest również używany do adnotacji funkcji — jawnie lub niejawnie za pomocą `_Return_type_success_` na podstawie elementu typedef. Gdy adnotacja `_On_failure_` jest obecna w parametrze funkcji lub wartości zwracanej, Każda adnotacja w `anno_list` (Anno) zachowuje się tak, jakby była zakodowana jako `_When_(!expr, anno)`, gdzie `expr` jest parametrem wymaganej adnotacji `_Success_`. Oznacza to, że implikowana aplikacja `_Success_` do wszystkich warunków post nie ma zastosowania do `_On_failure_`.|
|`_Return_type_success_(expr)`|Mogą być stosowane do elementu typedef. Wskazuje, że wszystkie funkcje, które zwracają ten typ i nie mają jawnie `_Success_` są adnotacjami, tak jakby były `_Success_(expr)`. nie można użyć `_Return_type_success_` w przypadku funkcji ani elementu typedef wskaźnika funkcji.|
|`_Success_(expr)`|`expr` jest wyrażeniem, które zwraca rvalue. Gdy adnotacja `_Success_` jest obecna w deklaracji lub definicji funkcji, Każda adnotacja (`anno`) w funkcji i w warunku jest zachowywać się tak, jakby była zakodowana jako `_When_(expr, anno)`. Adnotacja `_Success_` może być używana tylko w funkcji, nie na jej parametrach lub zwracanym typie. W funkcji może być co najwyżej jedna adnotacja `_Success_` i nie może ona być w żadnym `_When_`, `_At_` lub `_Group_`. Aby uzyskać więcej informacji, zobacz [Określanie, kiedy i gdzie ma zastosowanie adnotacja](../code-quality/specifying-when-and-where-an-annotation-applies.md).|
