...
Obiekt lokalny typu <xref:System.IDisposable> jest tworzony, ale obiekt nie jest usuwany, zanim wszystkie odwołania do obiektu nie należą do zakresu.
...
Można użyć [instrukcji`using`](/dotnet/csharp/language-reference/keywords/using-statement) ([`Using`](/dotnet/visual-basic/language-reference/statements/using-statement) w Visual Basic) do zawijania obiektów implementujących <xref:System.IDisposable>. Obiekty, które są opakowane w ten sposób, są automatycznie usuwane na końcu bloku `using`. Jednakże następujące sytuacje nie powinny być obsługiwane za pomocą instrukcji `using`:
...
- Gdy konstruktory chronione przez tylko jeden program obsługi wyjątków są zagnieżdżane w [części pozyskiwania instrukcji `using`](/dotnet/csharp/language-reference/language-specification/statements#the-using-statement), błąd w konstruktorze zewnętrznym może spowodować, że obiekt utworzony przez Konstruktor zagnieżdżony nigdy nie zostanie zamknięty. W poniższym przykładzie błąd w konstruktorze <xref:System.IO.StreamReader> może spowodować, że obiekt <xref:System.IO.FileStream> nigdy nie zostanie zamknięty. CA2000 flaguje naruszenie reguły w tym przypadku.
...
`tempPort` jest konstruowany i otwarty w bloku `try` i wszystkie inne wymagane zadania są wykonywane w tym samym bloku `try`. Na końcu bloku `try` otwarty port zostanie przypisany do obiektu `port`, który zostanie zwrócony, a obiekt `tempPort` jest ustawiony na `null`.
