...
Według Konwencji nazwy parametrów używają notacji CamelCase wielkości liter, a przestrzeń nazw, typ i nazwy elementów członkowskich używają wielkości liter w Pascalu. W nazwie notacji CamelCase — pierwsza litera jest małymi literami, a pierwsza litera dowolnego wyrazu w nazwie jest wielką literą. Przykłady nazw notacji CamelCase — `packetSniffer`, `ioFile`i `fatalErrorCode`. W nazwie z wielkością liter w języku Pascal pierwsza litera ma wielkie litery, a pierwsza litera dowolnego wyrazu w nazwie jest wielką literą. Przykłady nazw z wielkością liter w języku Pascal są `PacketSniffer`, `IOFile`i `FatalErrorCode`.
