...
Poniższy przykład przedstawia metodę, która narusza regułę przy użyciu C# `is` operatora. Druga metoda spełnia regułę przez zamianę `is` operatora z testem na wynik `as` operatora, co zmniejsza liczbę operacji rzutowania na iterację z dwóch do jednego. Trzecia metoda również spełnia regułę przy użyciu `is` [dopasowania wzorca](/dotnet/csharp/language-reference/keywords/is#pattern-matching-with-is) , aby utworzyć zmienną żądanego typu, jeśli konwersja typu powiedzie się.

[!code-csharp[FxCop.Performance.UnnecessaryCastsAsIs#1](../code-quality/codesnippet/CSharp/ca1800-do-not-cast-unnecessarily_1.cs)]

Poniższy przykład przedstawia metodę, `start_Click`, która ma wiele duplikatów jawnych, które naruszają regułę i `reset_Click`metodę,, która spełnia reguły przez przechowywanie rzutowania w zmiennej lokalnej.
