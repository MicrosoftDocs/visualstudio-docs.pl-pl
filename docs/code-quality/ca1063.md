...
- `Finalize` ponownie przesłonić.

- `Dispose()` został zastąpiony.

- Metoda `Dispose()` nie jest publiczna, [zapieczętowana](/dotnet/csharp/language-reference/keywords/sealed)ani nazywana **Dispose**.

- `Dispose(bool)` nie jest chroniony, wirtualny lub niezapieczętowany.

- W niezapieczętowanych typach `Dispose()` muszą wywoływać `Dispose(true)`.

- W przypadku typów niezapieczętowanych implementacja `Finalize` nie wywołuje ani obu `Dispose(bool)` lub finalizatora klasy bazowej.

Naruszenie jednego z tych wzorców wyzwala ostrzeżenie CA1063.

Każdy niezapieczętowany typ, który deklaruje i implementuje interfejs <xref:System.IDisposable>, musi zapewnić własną metodę `protected virtual void Dispose(bool)`. `Dispose()` powinna wywołać `Dispose(true)`, a finalizator powinien wywołać `Dispose(false)`. W przypadku utworzenia niezapieczętowanego typu, który deklaruje i implementuje interfejs <xref:System.IDisposable>, należy zdefiniować `Dispose(bool)` i wywołać go. Aby uzyskać więcej informacji, zobacz [Oczyszczanie zasobów niezarządzanych (Przewodnik po platformie .NET)](/dotnet/standard/garbage-collection/unmanaged) i [wzorca usuwania](/dotnet/standard/design-guidelines/dispose-pattern).
