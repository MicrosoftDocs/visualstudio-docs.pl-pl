...
W poniższym przykładzie metoda jest oznaczona przy użyciu atrybutu <xref:System.Security.SecuritySafeCriticalAttribute> i jest członkiem klasy, która jest oznaczona atrybutem <xref:System.Security.SecurityCriticalAttribute>. Bezpieczny atrybut zabezpieczeń powinien zostać usunięty.
