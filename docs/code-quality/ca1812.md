...
Jeśli zastosujesz <xref:System.Runtime.CompilerServices.InternalsVisibleToAttribute?displayProperty=fullName> do analizowanego zestawu, ta reguła nie będzie flagować typów, które są oznaczone jako [`internal`](/dotnet/csharp/language-reference/keywords/internal) ([`Friend` w Visual Basic](/dotnet/visual-basic/language-reference/modifiers/friend)), ponieważ pole może być używane przez zestaw zaprzyjaźniony.

## <a name="how-to-fix-violations"></a>Jak naprawić naruszenia

Aby naprawić naruszenie tej reguły, Usuń typ lub Dodaj kod, który go używa. Jeśli typ zawiera tylko metody `static`, należy dodać jeden z następujących elementów do typu, aby uniemożliwić kompilatorowi emitowanie domyślnego publicznego konstruktora wystąpień:

- Modyfikator `static` dla typów C# przeznaczonych dla .NET Framework 2,0 lub nowszych.
