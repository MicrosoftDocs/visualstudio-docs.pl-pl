...
Reguła CA1052 zakłada, że typ, który zawiera tylko statyczne składowe, nie jest przeznaczony do dziedziczenia, ponieważ typ nie udostępnia żadnych funkcji, które mogą zostać zastąpione w typie pochodnym. Typ, który nie powinien być dziedziczony, powinien być oznaczony za pomocą modyfikatora `static` w C# , aby uniemożliwić jego użycie jako typ podstawowy. Ponadto należy usunąć jego konstruktora domyślnego. W Visual Basic Klasa powinna być konwertowana na [moduł](/dotnet/visual-basic/language-reference/statements/module-statement).
...
Aby naprawić naruszenie tej zasady, Oznacz typ jako `static` i Usuń Konstruktor domyślny (C#) lub przekonwertuj go na moduł (Visual Basic).
