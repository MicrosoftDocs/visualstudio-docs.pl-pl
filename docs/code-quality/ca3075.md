...
> [!NOTE]
> Chociaż <xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType> jest zalecanym sposobem tworzenia wystąpienia <xref:System.Xml.XmlReader>, istnieją różnice dotyczące zachowania <xref:System.Xml.XmlTextReader>. @No__t_0 z <xref:System.Xml.XmlReader.Create%2A> normalizuje `\r\n` do `\n` w wartościach XML, podczas gdy <xref:System.Xml.XmlTextReader> zachowuje sekwencję `\r\n`.
