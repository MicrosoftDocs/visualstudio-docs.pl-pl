...
- Do sparametryzowanego konstruktora typu wyjątku jest przenoszona niepoprawna wartość argumentu String, który jest lub pochodzi od, <xref:System.ArgumentException>.
...
Sygnatury jednego i dwóch konstruktorów ciągów <xref:System.ArgumentException> i ich typów pochodnych nie są spójne w odniesieniu do `message` pozycji i parametrów `paramName`. Upewnij się, że te konstruktory są wywoływane z prawidłowymi argumentami ciągu. Podpisy są następujące:
