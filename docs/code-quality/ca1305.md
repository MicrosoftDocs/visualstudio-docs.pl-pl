...
Metoda lub Konstruktor wywołuje jeden lub więcej elementów członkowskich, które mają przeciążenia, które akceptują parametr <xref:System.IFormatProvider?displayProperty=fullName>, a metoda lub Konstruktor nie wywołuje przeciążenia, które przyjmuje parametr <xref:System.IFormatProvider>.
...
Aby naprawić naruszenie tej zasady, Użyj przeciążenia, które przyjmuje <xref:System.IFormatProvider> argument. Można też użyć [ C# ciągu interpolowanego](/dotnet/csharp/tutorials/string-interpolation) i przekazać go do metody <xref:System.FormattableString.Invariant%2A?displayProperty=nameWithType>.
