...
Aby naprawić naruszenia, wywołaj <xref:System.Threading.Tasks.Task.ConfigureAwait%2A> na <xref:System.Threading.Tasks.Task> oczekiwania. Dla parametru `continueOnCapturedContext` można przekazać wartość `true` lub `false`.
