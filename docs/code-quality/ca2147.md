...
Kod oznaczony jako <xref:System.Security.SecurityTransparentAttribute> nie ma wystarczających uprawnień do potwierdzenia.
...
Ten kod zakończy się niepowodzeniem, jeśli `SecurityTestClass` jest przezroczysty, gdy metoda `Assert` zgłasza <xref:System.InvalidOperationException>.
