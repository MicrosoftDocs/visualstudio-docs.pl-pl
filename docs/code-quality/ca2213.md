...
Typ jest odpowiedzialny za likwidację wszystkich niezarządzanych zasobów. Reguła CA2213 sprawdza, czy typ jednorazowy (to jest, który implementuje <xref:System.IDisposable>) `T` deklaruje `F` pola, które jest wystąpieniem `FT`typu. Dla każdego pola `F`, do którego przypisano obiekt utworzony lokalnie w ramach metod lub inicjatorów typu zawierającego `T`, reguła podejmie próbę zlokalizowania wywołania do `FT.Dispose`. Reguła przeszukuje metody wywoływane przez `T.Dispose` i niższy poziom (czyli metody wywoływane przez metody wywoływane przez `T.Dispose`).
...
Aby naprawić naruszenie tej reguły, wywołaj <xref:System.IDisposable.Dispose%2A> pól, które są typu, które implementują <xref:System.IDisposable>.
...
Poniższy fragment kodu przedstawia `TypeA` typu, który implementuje <xref:System.IDisposable>.

[!code-csharp[FxCop.Usage.IDisposablePattern#1](../code-quality/codesnippet/CSharp/ca2213-disposable-fields-should-be-disposed_1.cs)]

Poniższy fragment kodu przedstawia typ `TypeB`, który narusza zasady CA2213 przez zadeklarowanie pola `aFieldOfADisposableType` jako typ samodzielny (`TypeA`) i nie wywoływanie <xref:System.IDisposable.Dispose%2A> w polu.
