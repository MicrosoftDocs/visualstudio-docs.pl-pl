---
title: C26481
ms.date: 07/21/2017
ms.topic: conceptual
f1_keywords:
- C26481
helpviewer_keywords:
- C26481
ms.assetid: 4fd8694d-b45b-4163-b2d5-88c4889d00ed
author: mikeblome
ms.author: mblome
manager: markl
ms.workload:
- multiple
ms.openlocfilehash: 63ea59967b739a2dcabf3235944f7a3559aa7b91
ms.sourcegitcommit: 535ef05b1e553f0fc66082cd2e0998817eb2a56a
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 10/07/2019
ms.locfileid: "72016114"
---
# <a name="c26481-no_pointer_arithmetic"></a>C26481 NO_POINTER_ARITHMETIC
To sprawdzenie obsługuje reguły *I. 13: Nie przekazuj tablicy jako jednego wskaźnika @ no__t-0. Za każdym razem, gdy surowce wskaźniki są używane w operacjach arytmetycznych, powinny być zastępowane bezpieczniejszymi rodzajami buforów, takimi jak `span<T>` lub `vector<T>`.

## <a name="remarks"></a>Uwagi
- Ten test jest bardziej restrykcyjny niż I. 13: nie pomija typów zstring ani czstring.
- C26481 i C26485 pochodzą z reguł [profilu bezpieczeństwa granic](https://github.com/isocpp/CppCoreGuidelines/blob/master/CppCoreGuidelines.md) wdrożonych w pierwszej wersji narzędzia sprawdzania C++ podstawowych wytycznych. Są one stosowane do kategorii wskaźnik surowy, ponieważ ułatwiają one Uniknięcie niebezpiecznego używania pierwotnych wskaźników.
