...
Typ ogólny zawiera członka `static` (`Shared` w Visual Basic).

Domyślnie ta reguła sprawdza tylko typy widoczne na zewnątrz, ale [można to skonfigurować](#configurability).

## <a name="rule-description"></a>Opis reguły

Gdy jest wywoływana `static` składowej typu generycznego, argument typu musi być określony dla tego typu. Po wywołaniu wystąpienia ogólnego elementu członkowskiego, które nie obsługuje wnioskowania, dla elementu członkowskiego musi zostać określony argument typu. Składnia służąca do określania argumentu Type w tych dwóch przypadkach jest różna i łatwa do pomylenia, ponieważ następujące wywołania demonstrują:
