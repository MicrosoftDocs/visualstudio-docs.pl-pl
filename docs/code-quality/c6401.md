...
ms.locfileid: "72434131"
---
# <a name="c6401"></a>C6401
Ostrzeżenie C6401: użycie nazwy > \<function w domyślnych ustawieniach regionalnych do wykonania porównania bez uwzględniania wielkości liter w ciągu stałej \< >. Daje nieoczekiwane wyniki w ustawieniach regionalnych innych niż angielskie
 ...
 Aby przeprowadzić porównanie niezależne od ustawień regionalnych przy użyciu `CompareString` w systemie Windows XP, pierwszym parametrem powinna być stała `LOCALE_INVARIANT`; na przykład, aby przeprowadzić test niezależny od ustawień regionalnych, czy `pString` pasuje do plik1. gif ignorowanie wielkich/małych liter, użyj wywołania, takiego jak:
