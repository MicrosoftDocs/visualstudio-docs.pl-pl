...
**C++ Podstawowe wytyczne**: F. 23: Użyj NOT_NULL @ no__t-> 0T, aby wskazać, że wartość "null" nie jest prawidłową wartością

Jeśli kod kiedykolwiek sprawdza wartość null zmiennych wskaźnika, należy to zrobić spójnie i sprawdzić poprawność wskaźników we wszystkich ścieżkach. Czasami agresywne sprawdzanie pod kątem wartości null jest nadal lepsze niż możliwość twardej awarii w jednej ze skomplikowanych gałęzi. Idealny kod powinien zostać przekształcony w taki sposób, aby był mniej skomplikowany (przez podział na wiele funkcji) i aby polegać na znacznikach, takich jak GSL:: NOT_NULL (zobacz Wytyczne obsługiwane przez bibliotekę), aby izolować części algorytmu, które mogą bezpiecznie zalogować się do prawidłowych wartości wskaźnika. Reguła TEST_ON_ALL_PATHS ułatwia znalezienie miejsc, w których kontrole wartości null są niespójne (z tego względu założenia mogą wymagać przeglądu) lub rzeczywiste usterki, w których potencjalna wartość null może pomijać sprawdzanie wartości null w niektórych ścieżkach kodu.
