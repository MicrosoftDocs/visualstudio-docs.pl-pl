...
Te klasy są deklarowane jako `internal` (`Friend`, w Visual Basic) i deklarują Konstruktor prywatny, aby zapobiec tworzeniu nowych wystąpień. Metody w tych klasach powinny być `static` i `internal` (`Shared` i `Friend` w Visual Basic).
