...
## <a name="example"></a>Przykład
Następująca aplikacja korzysta z wcześniej zdefiniowanej biblioteki. Należy zauważyć, że typ, który jest zadeklarowany poza przestrzenią nazw jest tworzony, gdy nazwa `Test` nie jest kwalifikowana przez przestrzeń nazw. Należy również pamiętać, że aby uzyskać dostęp do typu `Test` w `Goodspace`, wymagana jest nazwa przestrzeni nazw.
