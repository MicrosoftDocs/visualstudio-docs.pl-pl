...
Aby naprawić naruszenie tej reguły, Zmień nazwy przeciążonych metod tak, aby nazwy były unikatowe. Alternatywnie, aby interfejs był niewidoczny dla modelu COM, należy zmienić dostępność na `internal` (`Friend` w [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)]) lub stosując atrybut <xref:System.Runtime.InteropServices.ComVisibleAttribute?displayProperty=fullName> ustawiony na `false`.
