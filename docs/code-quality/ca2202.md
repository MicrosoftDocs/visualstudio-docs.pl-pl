...
Aby rozwiązać ten problem, należy użyć bloku `try`/`finally` zamiast zewnętrznej instrukcji `using`. W bloku `finally` upewnij się, że `stream` zasób nie ma wartości null.
