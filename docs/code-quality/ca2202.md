...
Implementacja metody zawiera ścieżki kodu, które mogą spowodować wielokrotne wywołania <xref:System.IDisposable.Dispose%2A?displayProperty=fullName> lub równoważnej metody Dispose, takiej jak metoda Close () dla niektórych typów, w tym samym obiekcie.
...
Aby rozwiązać ten problem, należy użyć bloku `try` @ no__t-1 @ no__t-2 zamiast instrukcji Outer `using`. W bloku `finally` upewnij się, że zasób `stream` nie ma wartości null.
