...
|Flaga | Flagi|<xref:System.Enum?displayProperty=fullName>|
...
Jeśli uruchamiasz tę regułę z [analizatorów FxCop](install-fxcop-analyzers.md) (a nie ze starszą analizą), możesz skonfigurować listę dozwolonych sufiksów, przy czym każdy sufiks jest oddzielony znakiem kreski pionowej ("|"). Na przykład, aby określić, że reguła nie powinna być uruchamiana względem sufiksów flag i flag, Dodaj następującą parę klucz-wartość do pliku editorconfig w projekcie:
