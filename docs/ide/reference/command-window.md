...
W oknie **polecenia** znak równości (=) jest interpretowany jako operator porównania. Nie można używać operatorów przypisania w oknie **poleceń** . Tak więc, na przykład, jeśli wartości `varA` zmiennych i `varB` są różne, polecenie `>Debug.EvaluateStatement(varA=varB)` zwróci wartość `False`.
...
Jeśli przełączniki krótkie są łączone w grupę i mają daną wartość, ta wartość ma zastosowanie do każdego przełącznika. Na przykład `/pgm:123` jest równe `/p:123 /g:123 /m:123`. Błąd występuje, jeśli którykolwiek z przełączników w grupie nie akceptuje wartości.
