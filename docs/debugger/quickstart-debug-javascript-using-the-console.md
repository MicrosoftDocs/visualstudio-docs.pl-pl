...
4. W elemencie BODY elementu index. html Zastąp istniejący kod HTML tym kodem:
...
5. Otwórz domyślny. CSS i Dodaj arkusz CSS dla selektora `#fView`:
...
6. Otwórz program default. js i Zastąp kod następującym kodem JavaScript:
...
9. Po uruchomieniu aplikacji `FlipView` wpisz `Data.items` w wierszu polecenia okna konsoli (obok symbolu "> >") i naciśnij klawisz ENTER.

    Wizualizator dla obiektu `items` pojawi się w oknie konsoli. Oznacza to, że obiekt `items` skonkretyzowany i jest dostępny w bieżącym kontekście skryptu. W oknie konsoli możesz kliknąć węzeł obiektu, aby wyświetlić wartości właściwości (lub użyć klawiszy strzałek). Jeśli klikniesz przycisk w dół w obiekcie `items._data`, jak widać na ilustracji, zobaczysz, że odwołania do źródła obrazu są nieprawidłowe, zgodnie z oczekiwaniami. Obrazy domyślne (logo. png) nadal znajdują się w obiekcie i brakuje obrazów przeplatanych o oczekiwanych obrazach.

    ![Okno konsoli JavaScript](../debugger/media/js_console_window.png "JS_Console_Window")

    Należy również zauważyć, że w obiekcie `items._data` istnieje wiele więcej elementów niż oczekiwano.

10. W wierszu polecenia wpisz `Data.items.push` i naciśnij klawisz ENTER. Okno konsoli zawiera wizualizator dla funkcji `push`, która jest zaimplementowana w [!INCLUDE[winjs_long](../debugger/includes/winjs_long_md.md)] pliku projektu. W tej aplikacji używamy `push` do dodawania poprawnych elementów. Za pomocą funkcji IntelliSense dowiesz się, że należy użyć `setAt`, aby zastąpić domyślne obrazy.

11. Aby rozwiązać ten problem interaktywnie bez zatrzymywania sesji debugowania, Otwórz program default. js i wybierz ten kod z funkcji `updateImages`:
...
12. Popraw wywołania funkcji `push` w monicie, zastępując `pages.push` `Data.items.setAt`. Poprawiony kod powinien wyglądać następująco:
    ...
    > [!TIP]
    > Jeśli chcesz użyć obiektu `pages` zamiast `Data.items`, należy ustawić punkt przerwania w kodzie, aby zachować obiekt `pages` w zakresie.
...
16. Sprawdź aplikację ponownie i zobaczysz, że poprawne obrazy znajdują się na prawidłowych stronach `FlipView`.
...
18. Zatrzymaj debugowanie, wybierając pozycję **debuguj**  > **Zatrzymaj debugowanie** lub naciskając klawisze Shift + F5, a następnie Popraw kod źródłowy.

    Aby uzyskać pełną, domyślną stronę HTML zawierającą poprawiony przykładowy kod, zobacz [Debugowanie kodu HTML, CSS i JavaScript](../debugger/debug-html-css-and-javascript-sample-code.md).

## <a name="InteractiveDebuggingBreakMode"></a>Interaktywny tryb debugowania i przerwania
...
Gdy program jest w trybie przerwania, można użyć okna konsoli JavaScript do uruchomienia skryptów i poleceń, które są prawidłowe w bieżącym kontekście wykonywania skryptu. W tej procedurze zostanie użyta stała wersja aplikacji `FlipView` utworzonej wcześniej w celu zademonstrowania użycia trybu przerwania.

#### <a name="to-set-a-breakpoint-and-debug-the-app"></a>Aby ustawić punkt przerwania i debugować aplikację

1. W pliku default. html aplikacji `FlipView`, która została wcześniej utworzona, otwórz menu skrótów dla funkcji `updateImages()`, a następnie wybierz **punkt przerwania**  > **Wstaw punkt przerwania**.
...
3. Wybierz **debuguj**  > **Rozpocznij debugowanie**lub naciśnij klawisz F5.
    ...
    Aplikacja przechodzi w tryb przerwania, gdy wykonanie osiągnie funkcję `updateImages()`, a bieżący wiersz wykonania programu jest wyróżniony kolorem żółtym.
...
8. Naciśnij klawisz F5, aby kontynuować wykonywanie programu. Zostanie wyświetlona aplikacja `FlipView`, a teraz wszystkie cztery strony pokazują jeden z obrazów innych niż domyślne.

    Aby przełączyć się z powrotem do programu Visual Studio, naciśnij klawisz F12 lub Alt + Tab.

## <a name="SinglelineMultilineMode"></a>Tryb jednowierszowy i tryb wielowierszowy w oknie konsoli JavaScript
...
Okno konsoli JavaScript umożliwia korzystanie z jednego kontekstu wykonywania, który reprezentuje pojedyncze wystąpienie hosta platformy sieci Web (WWAHost. exe) w danym momencie. W niektórych scenariuszach aplikacja może uruchomić inne wystąpienie hosta, na przykład w przypadku używania `iframe`, kontraktu udziału, internetowego elementu roboczego lub kontrolki `WebView`. Jeśli uruchomione jest inne wystąpienie hosta, można wybrać inny kontekst wykonywania podczas uruchamiania aplikacji, wybierając kontekst wykonywania na liście **Target** .
