---
title: Ustawienia projektu dla konfiguracji C++ debugowania
ms.custom: seodec18
ms.date: 11/26/2018
ms.topic: reference
f1_keywords:
...
Ustawienia projektu dla konfiguracji C lub C++ Debug można zmienić w oknie dialogowym **strony właściwości** , zgodnie z opisem w temacie [How to: Set Debug and Release](../debugger/how-to-set-debug-and-release-configurations.md)Configurations. W poniższych tabelach przedstawiono, gdzie znaleźć ustawienia związane z debugerem w oknie dialogowym **strony właściwości** .

> [!NOTE]
> Ustawienia projektu debugowania w kategorii **Właściwości/debugowanie** są różne dla aplikacji platformy UWP oraz dla składników, które są zapisywane C++. Zobacz [Rozpoczynanie sesji debugowania (VB, C# C++ i XAML)](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md).
...
| **Typ debugera** | Określa typ debugera do użycia: **tylko natywny**, tylko **zarządzany**, **tylko procesor GPU**, **mieszany**, **Auto(domyślny** ) lub **skrypt**.<br /><br /> Tylko -   **Native** jest dla niezarządzanego C++ kodu.<br />Tylko -   **Managed** to kod, który jest uruchamiany w środowisku uruchomieniowym języka wspólnego (kod zarządzany).<br />-   **mieszane** wywołuje debugery dla kodu zarządzanego i niezarządzanego.<br />@no__t **-0 określa** typ debugera na podstawie informacji kompilatora i exe.<br />**skrypt** -    wywołuje debuger dla skryptów.<br />**procesor gpu** -    jest przeznaczony C++ tylko dla kodu amp, który działa na urządzeniu GPU lub w rasteryzacji odwołania DirectX. Zobacz [Debugowanie kodu GPU](../debugger/debugging-gpu-code.md). |
| **Środowisko** (lokalny debuger systemu Windows i zdalny debuger systemu Windows) | Określa zmienne środowiskowe dla debugowanego programu. Użyj standardowej składni zmiennej środowiskowej (na przykład `PATH="%SystemRoot%\..."`). Te zmienne zastępują środowisko systemowe lub są scalone ze środowiskiem systemowym, w zależności od ustawienia **środowisko scalone** . Po kliknięciu lewym przyciskiem myszy w kolumnie Ustawienia "Edytuj..." się. Wybierz ten link, aby edytować zmienne środowiskowe. |
...
| **Debugowanie SQL** (wszystkie oprócz debuger klastra MPI) | Umożliwia debugowanie procedur SQL z aplikacji [!INCLUDE[vcprvc](../code-quality/includes/vcprvc_md.md)]. Ustawienie domyślne to nie. |
...
| **Wdróż biblioteki C++ środowiska uruchomieniowego debugowania wizualnego** (Zdalny debuger systemu Windows) | Jeśli ustawiono właściwość katalogu wdrażania, określa, czy biblioteki środowiska uruchomieniowego C++ debugowania wizualizacji dla bieżącej platformy powinny zostać skopiowane do udziału sieciowego. Ustawieniem domyślnym jest tak. |

## <a name="cc-folder-general-category"></a>C/C++ folder (kategoria ogólna)
...
## <a name="cc-folder-optimization-category"></a>C/C++ folder (kategoria optymalizacji)
...
|**Zestaw możliwością debugowania** ([/ASSEMBLYDEBUG](/cpp/build/reference/mapinfo-include-information-in-mapfile))|Określa ustawienia dla opcji konsolidatora/ASSEMBLYDEBUG. Możliwe wartości to:<br /><br /> -   **nie wyemitowano atrybutu możliwością debugowania**.<br />**śledzenie środowiska uruchomieniowego -    i wyłączanie optymalizacji (/ASSEMBLYDEBUG)** . Jest to ustawienie domyślne,<br />-   **Brak śledzenia środowiska uruchomieniowego i Włącz optymalizacje (/ASSEMBLYDEBUG: Disable)** .<br />-    **\<inherit z ustawień domyślnych elementu nadrzędnego lub projektu >** .<br />-Aby uzyskać więcej informacji, zobacz [/ASSEMBLYDEBUG (Add DebuggableAttribute)](/cpp/build/reference/assemblydebug-add-debuggableattribute).|
...
- [Tworzenie projektów i C++ zarządzanie nimi](/cpp/ide/creating-and-managing-visual-cpp-projects)
