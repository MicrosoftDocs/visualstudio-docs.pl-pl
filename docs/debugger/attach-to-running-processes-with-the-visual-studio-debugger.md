...
W przypadku niektórych typów aplikacji, takich jak aplikacje universal Windows App (UWP), nie dołącza się bezpośrednio do nazwy procesu, ale zamiast tego należy użyć opcji menu **Debuguj** zainstalowany pakiet aplikacji w Visual Studio (zobacz tabelę).
...
|-|-|-|-|
|Zdalne debugowanie ASP.NET 4 lub 4.5 na serwerze usług IIS|Używanie narzędzi zdalnych i **dołączanie do procesu**|*w3wp.exe*|Zobacz [Zdalne debugowanie ASP.NET na zdalnym komputerze usług IIS](../debugger/remote-debugging-aspnet-on-a-remote-iis-7-5-computer.md)|
|Zdalne debugowanie ASP.NET Core na serwerze usług IIS|Używanie narzędzi zdalnych i **dołączanie do procesu**|*w3wp.exe* lub *dotnet.exe*|Począwszy od programu .NET Core 3, *w3wp.exe* jest używany dla domyślnego modelu [hostingu w aplikacji.](/aspnet/core/host-and-deploy/aspnet-core-module?view=aspnetcore-3.1&preserve-view=true#hosting-models) Aby uzyskać informacje na temat wdrażania aplikacji, [zobacz Publikowanie w usługach IIS.](/aspnet/core/host-and-deploy/iis/) Aby uzyskać bardziej szczegółowe informacje, zobacz Remote debugging ASP.NET Core on a remote IIS computer (Zdalne [debugowanie na zdalnym komputerze usług IIS)](../debugger/remote-debugging-aspnet-on-a-remote-iis-computer.md#BKMK_attach)|
...
|Debugowanie aplikacji w języku C#, Visual Basic lub C++ na maszynie lokalnej|Użyj standardowego debugowania **(F5)** lub **dołącz do procesu**|*\<appname>.exe*|W większości scenariuszy używaj debugowania standardowego, a nie **dołączania do procesu**.|
|Zdalne debugowanie aplikacji Windows klasycznej|Zdalne narzędzia|Nie dotyczy| Zobacz [Zdalne debugowanie aplikacji C# lub Visual Basic lub](../debugger/remote-debugging-csharp.md) Zdalne [debugowanie aplikacji języka C++](../debugger/remote-debugging-cpp.md)|
