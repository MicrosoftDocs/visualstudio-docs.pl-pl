---
title: Napisz wizualizator w C# | Microsoft Docs
ms.custom: seodec18
ms.date: 04/12/2019
ms.topic: conceptual
dev_langs:
...
1. Utwórz nowy projekt biblioteki klas.
    ...
    Naciśnij klawisz **ESC** , aby zamknąć okno uruchamiania. **Naciśnij klawisze CTRL + Q** , aby otworzyć pole wyszukiwania, **wpisz Biblioteka klas**, wybierz pozycję **Szablony**, a następnie wybierz pozycję **Utwórz nową bibliotekę klas (.NET standard)** . W wyświetlonym oknie dialogowym wybierz pozycję **Utwórz**.
    ::: moniker-end
    ::: moniker range="vs-2017"
    Na górnym pasku menu wybierz kolejno pozycje **plik**  > **Nowy**  > **projekt**. W lewym okienku okna dialogowego **Nowy projekt** w obszarze **Wizualizacja C#** wybierz **.NET Standard**, a następnie w środkowym okienku wybierz pozycję **Biblioteka klas (.NET standard)** .
    ::: moniker-end
...
4. W oknie dialogowym **Dodaj odwołanie** na karcie **Przeglądaj** wybierz pozycję **Przeglądaj** i Znajdź plik Microsoft. VisualStudio. DebuggerVisualizers. dll.
    ...
    Bibliotekę DLL można znaleźć w katalogu instalacyjnym programu *\<Visual Studio > \Common7\IDE\PublicAssemblies* podkatalog programu Visual Studio.

5. Kliknij przycisk **OK**.

6. W DebuggerSide.cs Dodaj następujące elementy do dyrektyw `using`:

   ```csharp
   using Microsoft.VisualStudio.DebuggerVisualizers;
   ```

   Teraz możesz przystąpić do tworzenia kodu po stronie debugera. Jest to kod, który działa w debugerze, aby wyświetlić informacje, które chcesz wizualizować. Najpierw należy zmienić deklarację obiektu `DebuggerSide`, aby dziedziczyć z klasy bazowej `DialogDebuggerVisualizer`.
...
- W `public class DebuggerSide` Dodaj następującą **metodę:**
  ...
  Metoda `Show` zawiera kod, który tworzy okno dialogowe wizualizatorer lub inny interfejs użytkownika i wyświetla informacje, które zostały przesłane do wizualizatora z debugera. Należy dodać kod, który tworzy okno dialogowe i wyświetla informacje. W tym instruktażu należy to zrobić przy użyciu okna komunikatu formularzy systemu Windows. Najpierw należy dodać dyrektywę Reference i `using` dla elementu System. Windows. Forms.
...
4. W DebuggerSide.cs Dodaj następujące elementy do dyrektyw `using`:
...
1. Dodaj następujący kod atrybutu do DebuggerSide.cs, po dyrektywie `using`, ale przed `namespace MyFirstVisualizer`:
...
2. W menu **kompilacja** wybierz polecenie **Kompiluj MyFirstVisualizer**. Projekt powinien zostać skompilowany pomyślnie. Popraw wszystkie błędy kompilacji przed kontynuowaniem.
   ...
   W tym momencie pierwszy wizualizator zostanie zakończony. Jeśli wykonano kroki prawidłowo, można skompilować wizualizator i zainstalować go w [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]. Przed zainstalowaniem wizualizatora w [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)], należy go przetestować, aby upewnić się, że działa poprawnie. Teraz utworzysz zespół testów do uruchamiania wizualizatora bez instalowania go w [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)].
...
1. W Eksplorator rozwiązań kliknij prawym przyciskiem myszy rozwiązanie, wybierz polecenie **Dodaj**, a następnie kliknij pozycję **Nowy projekt**.
    ...
    ::: moniker-end
    ::: moniker range="vs-2017"
    Na górnym pasku menu wybierz kolejno pozycje **plik**  > **Nowy**  > **projekt**. W lewym okienku okna dialogowego **Nowy projekt** w obszarze **Wizualizacja C#** wybierz pozycję **Windows Desktop**, a następnie w środkowym okienku wybierz pozycję **aplikacja konsoli (.NET Framework)** .
    ::: moniker-end
...
3. W TestConsole.cs Dodaj następujący kod do dyrektyw `using`:
...
2. W menu **debugowanie** wybierz polecenie **Uruchom**.
   ...
   Nabycia. Właśnie został skompilowany i przetestowany pierwszy wizualizator.

   Jeśli chcesz użyć wizualizatora w [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] zamiast tylko wywołać go z zespołu testowego, musisz go zainstalować. Aby uzyskać więcej informacji, zobacz [How to: Install the wizualizator](../debugger/how-to-install-a-visualizer.md).
...
2. W oknie dialogowym **Nowy projekt** w obszarze **Wizualizacja C#** wybierz pozycję **.NET Standard**.
...
3. W oknie dialogowym **Dodaj nowy element** w obszarze **elementy wizualne C#** wybierz opcję **wizualizator debugera**.
