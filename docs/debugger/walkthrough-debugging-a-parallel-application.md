...
## <a name="c-sample"></a>C#Northwind
 Jeśli używasz C# przykładu, w tym instruktażu założono również, że kod zewnętrzny jest ukryty. Aby sprawdzić, czy jest wyświetlany kod zewnętrzny, kliknij prawym przyciskiem myszy **nazwę** nagłówka tabeli okna **stosu wywołań** , a następnie zaznacz lub wyczyść pole **Pokaż kod zewnętrzny**. Jeśli ta funkcja nie zostanie ustawiona, można nadal korzystać z tego instruktażu, ale wyniki mogą się różnić od ilustracji.

## <a name="c-sample"></a>C++Northwind
 Jeśli używasz C++ przykładu, możesz zignorować odwołania do kodu zewnętrznego w tym temacie. Kod zewnętrzny dotyczy tylko C# przykładu.

## <a name="illustrations"></a>Rysunk
 Ilustracje w tym temacie zostały zarejestrowane na komputerze z czterema rdzeniami, na których jest C# uruchomiony przykład. Mimo że można użyć innych konfiguracji do wykonania tego instruktażu, ilustracje mogą się różnić od tego, co jest wyświetlane na komputerze.
...
## <a name="using-the-parallel-stacks-window-threads-view"></a>Korzystanie z okna stosów równoległych: Widok wątków
 W menu **debugowanie** kliknij **Rozpocznij debugowanie**. Poczekaj, aż pierwszy punkt przerwania zostanie osiągnięty.
...
1. W menu **debugowanie** wskaż polecenie **Windows** , a następnie kliknij pozycję **stosy równoległe**. Upewnij się, że w polu w lewym górnym rogu zaznaczone są **wątki** .
     ...
     Można ustawić, ile szczegółów ma być pokazywanych dla ramek stosu (**nazw modułów**, **typów parametrów**, **nazw parametrów**, **wartości parametrów**, **numerów wierszy** i **przesunięć bajtów**) przez kliknięcie prawym przyciskiem myszy w **stosie wywołań** . Dział.
...
1. Aby wznowić wykonywanie do momentu, gdy drugi punkt przerwania zostanie osiągnięty, w menu **debugowanie** kliknij przycisk **Kontynuuj**.
     ...
     Zadanie 4, z kolei czeka na monitor należący do wątku przypisanego do zadania 2. (Kliknij prawym przyciskiem myszy wiersz nagłówka i wybierz **kolumny** > **przypisanie wątku** , aby wyświetlić wartość przypisania wątku dla zadania 2).
...
## <a name="summary"></a>Podsumowanie
 W tym instruktażu przedstawiono okna debugera **zadań równoległych** i **stosów równoległych** . Użyj tych okien dla rzeczywistych projektów, które używają kodu wielowątkowego. Można przeanalizować kod równoległy C++zapisany C#w, lub Visual Basic.
