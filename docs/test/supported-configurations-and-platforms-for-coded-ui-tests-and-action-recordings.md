...
Obsługiwane konfiguracje i platformy dla kodowanych testów interfejsu użytkownika dla Visual Studio Enterprise są wymienione w poniższej tabeli. Te konfiguracje mają zastosowanie również do nagrań akcji utworzonych [!INCLUDE[MTRlong](../test/includes/mtrlong_md.md)]przy użyciu programu.
...
| Architektura | x86 i x64 **:**  Program Internet Explorer nie jest obsługiwany w trybie 64-bitowym, z [!INCLUDE[win8](../debugger/includes/win8_md.md)] wyjątkiem sytuacji, gdy program działa w wersji lub nowszej. |
| .NET | .NET 2.0, 3.0, 3.5, 4 i 4.5. **Uwaga:** [!INCLUDE[TCMext](../misc/includes/tcmext_md.md)] program Visual Studio wymaga programu .NET 4 do działania. Jednak aplikacje opracowane za pomocą wymienionych wersji środowiska .NET są obsługiwane. |
