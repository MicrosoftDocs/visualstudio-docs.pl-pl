...
Narzędzie analizy pokrycia kodu w programie Visual Studio zbiera dane dla zestawów natywnych i zarządzanych ( *.dll* lub *.exe* plików). Jednak w niektórych przypadkach w oknie **wyników pokrycia kodu** jest wyświetlany komunikat o błędzie podobny do "wygenerowane puste wyniki:...". Istnieje kilka powodów, dla których można uzyskać puste wyniki. Ten artykuł pomaga w rozwiązaniu tych problemów.
