...
Analiza &mdash;Check oknie danych wyjściowych. Na liście rozwijanej **Pokaż dane wyjściowe z** wybierz pozycję **testy**. Sprawdź, czy są zarejestrowane jakieś błędy lub ostrzeżenia.

Wyjaśnienie &mdash;Code analizy pokrycia jest wykonywane, gdy testy są uruchomione. Zawiera ona tylko te zestawy, które są ładowane do pamięci podczas wykonywania testów. Jeśli żaden z testów nie zostanie wykonany, nie ma niczego do zaraportowania.

Aby sprawdzić, czy testy zostały wykonane pomyślnie, należy wybrać polecenie **Uruchom wszystkie** w Eksploratorze testów. &mdash;In Napraw wszystkie błędy przed użyciem **analizy pokrycia kodu**.
...
Analiza &mdash;Open folderu docelowego kompilacji (zazwyczaj *bin\Debug*) i sprawdź, czy dla każdego zestawu znajduje się plik *. pdb* w tym samym katalogu, w którym znajduje się plik. *dll* lub *. exe* .

Wyjaśnienie &mdash;The aparat pokrycia kodu wymaga, aby każdy zestaw miał skojarzony z nim plik *. pdb* podczas przebiegu testu. Jeśli nie istnieje plik *. pdb* dla określonego zestawu, zestaw nie jest analizowany.

Plik *. pdb* musi być wygenerowany z tej samej kompilacji co pliki *. dll* lub *. exe* .

Rozdzielczość &mdash;Make upewnij się, że ustawienia kompilacji generują plik *. pdb* . Jeśli pliki *. pdb* nie są aktualizowane po skompilowaniu projektu, Otwórz właściwości projektu, wybierz stronę **kompilacja** , wybierz pozycję **Zaawansowane**i sprawdź **informacje debugowania**.

W C++ przypadku projektów upewnij się, że wygenerowane pliki. pdb mają pełne informacje o debugowaniu. Otwórz właściwości projektu i sprawdź, czy program **konsolidator**  > **debugowanie**  > **Generuj** informacje o debugowaniu, aby **generować informacje debugowania zoptymalizowane pod kątem udostępniania i publikowania (/Debug: Full)** .
...
Analiza &mdash;Determine, jeśli plik binarny został poddany dowolnej formie zaawansowanej optymalizacji, takiej jak optymalizacja profilowana, lub został Instrumentacją przez narzędzie profilowania, takie jak *VSInstr. exe* lub *vsperfmon. exe*.

Wyjaśnienie &mdash;If zestaw został już Instrumentacją lub zoptymalizowany przy użyciu innego narzędzia profilowania, zestaw został pominięty z analizy pokrycia kodu. Nie można przeprowadzić analizy pokrycia kodu na takich zestawach.

Rozdzielczość &mdash;Switch Wycofaj optymalizację i użyj nowej kompilacji.

### <a name="code-is-not-managed-net-or-native-c-code"></a>Kod nie jest zarządzany (.NET) lub nie jest kodem natywnym (C++)

Analiza &mdash;Verify, że uruchamiasz niektóre testy na serwerze zarządzanym lub C++ kodzie.

Wyjaśnienie &mdash;Code analizy pokrycia w programie Visual Studio jest dostępne tylko w kodzie zarządzanymC++i natywnym (). Jeśli pracujesz z narzędziami innych firm, niektóre lub wszystkie kody mogą być wykonywane na innej platformie.

Rozwiązanie &mdash;None dostępne.

### <a name="assembly-has-been-installed-by-ngen"></a>Zestaw został zainstalowany przez NGen

Analiza &mdash;Verify, że zestaw nie został załadowany z pamięci podręcznej obrazów natywnych.

Wyjaśnienie &mdash;For ze względu na wydajność, zestawy obrazów natywnych nie są analizowane. Aby uzyskać więcej informacji, zobacz [Ngen. exe (Generator obrazu natywnego)](/dotnet/framework/tools/ngen-exe-native-image-generator).

Rozwiązanie &mdash;Use wersji MSIL zestawu. Nie Przetwarzaj go za pomocą narzędzia NGen.

### <a name="custom-runsettings-file-with-bad-syntax"></a>Niestandardowy plik .runsettings o złej składni

Analiza &mdash;If używasz pliku Custom *. runsettings* , który może zawierać błąd składniowy. Pokrycie kodu nie jest uruchamiane, a okno pokrycia kodu nie jest otwarte na końcu przebiegu testu lub zawiera stare wyniki.

Wyjaśnienie &mdash;You można uruchomić testy jednostkowe z niestandardowym plikiem *. runsettings* , aby skonfigurować opcje pokrycia kodu. Opcje te umożliwiają uwzględnianie lub wykluczanie plików. Aby uzyskać więcej informacji, zobacz [Dostosowywanie analizy pokrycia kodu](../test/customizing-code-coverage-analysis.md).

@No__t_0There rozdzielczości są dwa możliwe typy błędów:
...
Analiza &mdash;If używasz pliku Custom *. runsettings* , upewnij się, że zawiera on zestaw.

Wyjaśnienie &mdash;You można uruchomić testy jednostkowe z niestandardowym plikiem *. runsettings* , aby skonfigurować opcje pokrycia kodu. Opcje te umożliwiają uwzględnianie lub wykluczanie plików. Aby uzyskać więcej informacji, zobacz [Dostosowywanie analizy pokrycia kodu](../test/customizing-code-coverage-analysis.md).

Rozwiązanie &mdash;Remove wszystkie węzły `Include` z pliku *. runsettings* , a następnie usuń wszystkie węzły `Exclude`. Jeśli to rozwiąże problem, umieść je z powrotem w etapach.
...
Analiza &mdash;In statycznie połączony kod natywny, część funkcji inicjującej **DllMain** i kod, który wywołuje, jest czasami wyświetlana jako niepokryte, nawet jeśli kod został wykonany.

Wyjaśnienie &mdash;The narzędzia pokrycia kodu działa przez wstawianie instrumentacji do zestawu tuż przed uruchomieniem aplikacji. W każdym załadowanym wcześniej zestawie kod inicjujący w **DllMain** jest wykonywany zaraz po załadowaniu zestawu i przed uruchomieniem aplikacji. Ten kod wydaje się nie być objęty, co zwykle dotyczy zestawów ładowanych statycznie.

@No__t_0None rozwiązania.

## <a name="see-also"></a>Zobacz także

- [Użyj pokrycia kodu, aby określić, jaka część kodu jest testowana](../test/using-code-coverage-to-determine-how-much-code-is-being-tested.md)
