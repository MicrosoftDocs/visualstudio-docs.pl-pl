...
ms.locfileid: "68925454"
---
# <a name="walkthrough-creating-a-wcf-data-service-with-wpf-and-entity-framework"></a>Przewodnik: Tworzenie usługi danych WCF w technologii WPF i Entity Framework
W tym instruktażu pokazano, jak utworzyć [!INCLUDE[ss_data_service](../data-tools/includes/ss_data_service_md.md)] prostą, która jest hostowana w aplikacji sieciWeb,anastępnieuzyskaćdoniejdostępzaplikacjiWindowsForms.[!INCLUDE[vstecasp](../code-quality/includes/vstecasp_md.md)]

W tym instruktażu:

- Utwórz aplikację sieci Web do hostowania [!INCLUDE[ss_data_service](../data-tools/includes/ss_data_service_md.md)]a.

- Utwórz element [!INCLUDE[adonet_edm](../data-tools/includes/adonet_edm_md.md)] `Customers` reprezentujący tabelę w bazie danych Northwind.
...
Aby utworzyć [!INCLUDE[ss_data_service](../data-tools/includes/ss_data_service_md.md)], należy dodać projekt sieci Web, [!INCLUDE[adonet_edm](../data-tools/includes/adonet_edm_md.md)]utworzyć, a następnie utworzyć usługę z modelu.
...
2. W oknie dialogowym **Nowy projekt** rozwiń węzły **Visual Basic** lub wizualizacji **C#** i **sieci Web** , a następnie wybierz szablon **aplikacja sieci Web ASP.NET** .
...
W następnym kroku [!INCLUDE[adonet_edm](../data-tools/includes/adonet_edm_md.md)] utworzysz, który `Customers` reprezentuje tabelę w bazie danych Northwind.
...
7. Na pasku **adresu** wpisz `Customers` na końcu adresu URL dla **NorthwindCustomers. svc**, a następnie wybierz klawisz **Enter** .
     ...
     Zostanie wyświetlona Reprezentacja XML danych `Customers` w tabeli.
...
W następnym kroku dodasz odwołanie do usługi do [!INCLUDE[ss_data_service](../data-tools/includes/ss_data_service_md.md)] elementu w projekcie sieci Web.
...
2. W projektancie formularzy **Form1. vb** lub **Form1.cs** Otwórz menu skrótów i wybierz polecenie **Wklej**.
    ...
    Kontrolka, składnik i <xref:System.Windows.Forms.BindingNavigator> składnik są dodawane do formularza. <xref:System.Windows.Forms.BindingSource> <xref:System.Windows.Forms.DataGridView>

3. Wybierz kontrolkę **customersDataGridView** , a następnie w oknie **Właściwości** ustaw właściwość **Dock** na **Fill**.

4. W **Eksplorator rozwiązań**Otwórz menu skrótów dla węzła **Form1** i wybierz polecenie **Wyświetl kod** , aby otworzyć Edytor kodu, i Dodaj następującą `Imports` instrukcję lub `Using` na początku pliku:
...
2. Dodaj kontrolkę i kontrolkę z przybornika do formularza. <xref:System.Windows.Forms.Button> <xref:System.Windows.Forms.TextBox>

3. Otwórz menu skrótów dla <xref:System.Windows.Forms.Button> kontrolki, wybierz polecenie **Wyświetl kod**, a następnie Dodaj `Button1_Click` następujący kod do programu obsługi zdarzeń:
...
4. W poprzednim kodzie Zastąp `http://localhost:53161/NorthwindCustomers.svc` ciąg adresem URL `Form1_Load` z programu obsługi zdarzeń.
