...
Gdy przeciągasz procedury składowane i funkcje z **Eksplorator serwera** lub **Eksplorator bazy danych** do **projektanta O/R**, zwracany typ wygenerowanej metody <xref:System.Data.Linq.DataContext> różni się w zależności od tego, gdzie porzucasz element. Porzucenie elementów bezpośrednio do istniejącej klasy Entity tworzy metodę <xref:System.Data.Linq.DataContext> z typem zwracanym klasy Entity; upuszczanie elementów do pustego obszaru **projektanta o/R** (w obu okienka) tworzy metodę <xref:System.Data.Linq.DataContext>, która zwraca typ wygenerowany automatycznie. Typ wygenerowany automatycznie ma nazwę zgodną z nazwą procedury składowanej lub funkcją oraz właściwościami, które mapują do pól zwracanych przez procedurę składowaną lub funkcję.
