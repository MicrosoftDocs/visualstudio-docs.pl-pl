...
Narzędzia języka specyficznego dla domeny (narzędzia DSL), które są hostowane w [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], umożliwiają zaprojektowanie języka specyficznego dla domeny, a następnie wygenerowanie wszystkich elementów, które muszą być wymagane przez użytkowników w celu utworzenia modeli opartych na języku.
...
- Biblioteka DSL
  ...
  Kreator tworzy rozwiązanie [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], które ma następujące projekty:
...
- **DslPackage**
   ...
   Projekt DslPackage określa, jak narzędzia języka integrują się z [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].
...
## <a name="distributing-your-dsl-solution"></a>Dystrybuowanie rozwiązania DSL
 Narzędzia DSL generują pakiet, który jest hostowany w [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. Pakiet wyświetla Przybornik, Eksplorator DSL i inne elementy interfejsu użytkownika, które umożliwiają użytkownikom tworzenie modeli przy użyciu języka specyficznego dla domeny.

 Podczas kompilowania i uruchamiania rozwiązania narzędzi DSL w [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], drugie wystąpienie [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] pokazuje, jak język specyficzny dla domeny będzie wyglądał użytkownikowi w języku. Po zweryfikowaniu, że wszystko działa prawidłowo, można rozpowszechnić plik `.vsix`, który będzie znajdował się w folderze Build projektu DslPackage. Ten plik może służyć do instalowania programu DSL jako rozszerzenia [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] na innych komputerach.  Aby uzyskać więcej informacji, zobacz [wdrażanie rozwiązań językowych właściwych dla domeny](../modeling/deploying-domain-specific-language-solutions.md).
