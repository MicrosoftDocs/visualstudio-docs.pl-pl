...
## <a name="signature-of-an-operation"></a>Podpis operacji
 ...
 `: ReturnType` jest pomijane, jeśli nie zdefiniowano typu zwracanego.
...
|   **Parametry**    |    dawaj    |      Lista o <em>nazwie</em>**formularza:** <em>wpisz</em> **,** <em>name</em> **:** <em>Type</em> **,....** Kliknij przycisk **[...]** , aby edytować listę.<br /><br /> Typy mogą być typami pierwotnymi lub typami, które są zdefiniowane w modelu. W przypadku wprowadzenia nazwy nowego typu w tej właściwości, typ zostanie dodany do sekcji **nieokreślone typy** w EKSPLORATORZE modelu UML.      |
|   **Zwracany typ**   |    dawaj    |                                                                               **(brak)** lub typ pierwotny lub typ, który jest zdefiniowany w modelu. W przypadku wprowadzenia nazwy nowego typu w tej właściwości, typ zostanie dodany do sekcji **nieokreślone typy** w EKSPLORATORZE modelu UML.                                                                                |
| **Warunki końcowe**  |    dawaj    |                                                                                                                         Opcjonalny warunek określający relację między stanem systemu przed i po wykonaniu operacji.                                                                                                                         |
|  **Warunki wstępne**  |    dawaj    |                                                                                                                            Opcjonalny warunek określający założenia stanu systemu przed rozpoczęciem wykonywania operacji.                                                                                                                            |
| **Warunki treści** |    dawaj    |                                                                                                                                                       Opcjonalne ograniczenie wartości zwracanych przez operację.                                                                                                                                                       |
|   **Propagowan**    |    Public    |                  Dozwolone wartości i znaki, które pojawiają się w podpisie:<br /><br /> **I publicznie** widoczne globalnie<br /><br /> **-Private** -niewidoczny poza typem będącym właścicielem<br /><br /> **# Protected** -Visible do typów pochodzących od właściciela<br /><br /> **~ Pakiet** jest widoczny dla innych typów w tym samym pakiecie.                   |
|    **Podpisane**    |  *nazwa*+ ()   |                                                                                      Podsumowuje widoczność, nazwę, parametry i zwracany typ tej operacji. Te właściwości można zmienić, edytując podpis na diagramie lub edytując poszczególne właściwości.                                                                                      |
...
|   **Współbieżność**   |  Okreola  | **Sekwencyjnie** — operacja jest lub zostanie zaprojektowana bez kontroli współbieżności. Wywołanie tej operacji współbieżnie może spowodować błędy.<br /><br /> **Chroniona** — operacja zostanie automatycznie zablokowana do momentu ukończenia innych wystąpień.<br /><br /> **Współbieżne** — operacja jest zaprojektowana tak, aby wiele wywołań było wykonywanych jednocześnie. |
|    **Jest statyczny**    |    False     |                                                                                                  W przypadku wartości true ta operacja jest udostępniana między wszystkimi wystąpieniami tego typu.<br /><br /> Jeśli wartość jest równa true, nazwa operacji zostanie podkreślona, gdzie pojawia się na diagramie.                                                                                                   |
|   **Jest abstrakcyjny**   |    False     |                                                                                                                                        Jeśli wartość jest równa true, żaden kod nie jest skojarzony z tą operacją. W związku z tym Klasa będąca właścicielem jest abstrakcyjna.                                                                                                                                         |
|     **Jest liściem**     |    False     |                                                                                                                                              Projektant zaplanuje, że tej operacji nie można zastąpić w klasach pochodnych.                                                                                                                                              |
|    **Jest kwerendą**     |    False     |                                                                                                 W przypadku wartości true ta operacja nie powoduje wprowadzenia znaczących zmian stanu systemu. W związku z tym można go użyć na przykład w teście w celu sprawdzenia stanu systemu.                                                                                                  |
|  **Liczebność**   |      1       |                                 **1** — pojedyncza wartość określonego typu.<br /><br /> **0.. 1** — może być `null`.<br /><br /> \* — Kolekcja wartości określonego typu.<br /><br /> **1.. \\** \* — Kolekcja zawierająca co najmniej jedną wartość.<br /><br /> *n* `..` *m* -kolekcja zawierająca między `n` i `m` wartościami.                                  |
|   **Jest uporządkowany**    |    False     |                                                                                                                                             W przypadku wartości true kolekcja tworzy listę sekwencyjną. **Liczebność** jest większa niż 1.                                                                                                                                              |
|    **Jest unikatowy**    |    False     |                                                                                                                                         W przypadku opcji true w kolekcji nie ma zduplikowanych wartości. **Liczebność** jest większa niż 1.                                                                                                                                         |
