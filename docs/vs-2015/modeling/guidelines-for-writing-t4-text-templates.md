---
title: Wskazówki dotyczące pisania szablonów tekstowych T4 | Microsoft Docs
ms.date: 11/15/2016
...
- Klasy powinny być generowane w parach, po jednym dziedziczeniu. Klasa bazowa powinna zawierać wszystkie wygenerowane metody i właściwości, a Klasa pochodna powinna zawierać tylko konstruktory. Pozwala to na ręczne pisanie kodu w celu przesłonięcia dowolnej z wygenerowanych metod.
  ...
  Użyj folderu wygenerowanego kodu Umieść szablony i wygenerowane pliki w folderze projektu o nazwie **wygenerowany kod**, aby wyczyścić, że nie są to pliki, które powinny być edytowane bezpośrednio. W przypadku tworzenia niestandardowego kodu do przesłonięcia lub dodania do wygenerowanych klas należy umieścić te klasy w folderze o nazwie **kod niestandardowy**. Struktura typowego projektu wygląda następująco:
...
## <a name="guidelines-for-run-time-preprocessed-t4-templates"></a>Wytyczne dotyczące czasu wykonywania (wstępnie przetworzonych) szablonów T4
 Przenieś wspólny materiał do dziedziczonych szablonów można używać dziedziczenia do udostępniania metod i bloków tekstu między szablonami tekstu T4. Aby uzyskać więcej informacji, zobacz [dyrektywa dotycząca szablonu T4](../modeling/t4-template-directive.md).
...
Wiele modeli jest najbardziej efektywnie prezentowanych i zarządzanych po prostu jako tabele tekstowe, zwłaszcza jeśli są bardzo duże.
 ...
 Diagramy klas UML i działań często można dostosować do tych celów. Możesz również projektować własny typ diagramu jako język specyficzny dla domeny (DSL). Kod można generować z poziomu języka UML i językami DSL. Aby uzyskać więcej informacji, zobacz [analizowanie i modelowanie architektury](../modeling/analyze-and-model-your-architecture.md) oraz [analizowanie i modelowanie architektury](../modeling/analyze-and-model-your-architecture.md).

## <a name="see-also"></a>Zobacz też
 [Generowanie kodu w czasie projektowania przy użyciu szablonów](../modeling/design-time-code-generation-by-using-t4-text-templates.md) tekstowych w [czasie wykonywania Generowanie tekstu za pomocą szablonów tekstowych T4](../modeling/run-time-text-generation-with-t4-text-templates.md)
