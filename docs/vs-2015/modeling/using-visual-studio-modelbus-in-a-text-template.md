---
title: Używanie ModelBus w szablonie tekstowym | Microsoft Docs
ms.date: 11/15/2016
...
## <a name="creating-a-model-bus-adapter-for-access-from-text-templates"></a>Tworzenie karty magistrali modelu na potrzeby dostępu z szablonów tekstowych
 Aby można było rozpoznać odwołanie ModelBus w szablonie tekstowym, docelowy DSL musi mieć zgodną kartę. Szablony tekstowe są wykonywane w oddzielnym elemencie AppDomain z [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] edytorów dokumentów, dlatego Adapter musi ładować model zamiast uzyskiwać do niego dostęp za pośrednictwem DTE.

#### <a name="to-create-a-modelbus-adapter-that-is-compatible-with-text-templates"></a>Aby utworzyć kartę ModelBus zgodną z szablonami tekstu

1. Jeśli docelowe rozwiązanie DSL nie ma projektu **ModelBusAdapter** , utwórz je za pomocą Kreatora rozszerzenia ModelBus:

    1. Pobierz i zainstaluj [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] rozszerzenie ModelBus, jeśli jeszcze tego nie zrobiono. Aby uzyskać więcej informacji, zobacz temat [Wizualizacja i Modeling SDK](https://www.visualstudio.com/).
...
## <a name="writing-a-text-template-that-can-resolve-modelbus-references"></a>Pisanie szablonu tekstu, który może rozwiązać odwołania ModelBus
...
- Dołącz `hostspecific="true"` do dyrektywy Template.
...
```
 ...
 Po wykonaniu tego szablonu tekstu `SourceDsl` dyrektywa ładuje plik `Sample.source` . Szablon może uzyskać dostęp do elementów tego modelu, rozpoczynając od `this.ModelRoot` . Kod może używać klas domen i właściwości tego języka DSL.
...
1. Utwórz dwa językami DSL. Jeden DSL, *konsument*, ma `ModelBusReference` Właściwość, która może odwoływać się do innego DSL, *dostawcy*.
...
2. Na diagramie definicji DSL kliknij prawym przyciskiem myszy pustą część diagramu, która nie znajduje się blisko góry, a następnie kliknij pozycję **Włącz ModelBus**.
   ...
   - Jeśli nie widzisz **ModelBus Enable**, musisz pobrać i zainstalować rozszerzenie VMSDK ModelBus. Znajdź ją w witrynie VMSDK: [Wizualizacja i Modeling SDK](https://www.visualstudio.com/).
...
#### <a name="to-create-a-modelbus-adapter-that-is-configured-for-text-templates"></a>Aby utworzyć adapter ModelBus skonfigurowany do szablonów tekstowych
...
5. W `DslPackage` projekcie Dodaj odwołanie do projektu `T4ModelBusAdapter` .

6. W DslPackage\source.extension.tt Dodaj następujący wiersz poniżej `<Content>` .
...
10. Ponownie skompiluj rozwiązanie. Kliknij klawisz F5.

11. Sprawdź, czy DSL działa, naciskając klawisz F5. W projekcie eksperymentalnym Otwórz `Sample.provider` . Zamknij wystąpienie eksperymentalne [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] .
...
4. W klasie `ExampleElement` Dodaj nową właściwość domeny `MBR` , a w okno właściwości ustaw jej typ na `ModelBusReference` .
...
5. Kliknij prawym przyciskiem myszy właściwość domeny na diagramie, a następnie kliknij pozycję **Edytuj właściwości specyficzne dla ModelBusReference**. W oknie dialogowym wybierz **element modelu**.
    ...
    Podciąg po "&#124;" to filtr okna dialogowego Wybór pliku. Można ustawić, aby zezwalać na dowolnych plików przy użyciu *.\*
...
1. W rozwiązaniu MBConsumer naciśnij klawisze CTRL + F5. Doświadczalne wystąpienie otwiera się [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] w projekcie **MBConsumer\Debugging** .
...
4. Kliknij jeden przykład kształtu, a następnie w okno właściwości kliknij pozycję **[...]** we właściwości MBR. W oknie dialogowym kliknij przycisk **Przeglądaj** i wybierz opcję `Sample.provide` . W oknie elementy rozwiń zadanie typ i wybierz jeden z elementów.
...
1. W eksperymentalnym wystąpieniu programu [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] Otwórz przykładowy plik szablonu tekstowego. Ustaw jej zawartość w następujący sposób.
    ...
    1. `hostSpecific` `inherits` `template` Należy ustawić atrybuty i dyrektywy.

    2. Model konsumenta jest dostępny w zwykły sposób przez procesor dyrektywy, który został wygenerowany w ramach tego języka DSL.

    3. Dyrektywy Assembly i import muszą mieć możliwość dostępu do ModelBus i typów dostawcy DSL.

    4. Jeśli wiesz, że wiele MBRs jest połączonych z tym samym modelem, lepszym rozwiązaniem jest wywołanie metody tylko jeden raz.
...
1. Zamknij wystąpienie eksperymentalne [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] , jeśli jest uruchomione.
...
4. W eksperymentalnym wystąpieniu programu [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] Otwórz program `Debugging\Sample.consume` .
