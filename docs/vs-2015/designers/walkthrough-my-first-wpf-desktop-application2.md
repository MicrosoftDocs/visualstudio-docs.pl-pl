...
a Name = "Introduction" ></a> ten przewodnik zawiera wprowadzenie do programowania Windows Presentation Foundation (WPF). Utworzysz podstawową aplikację obejmującą elementy, które są wspólne dla większości aplikacji klasycznych WPF: oznakowanie XAML, powiązane z kodem, definicje aplikacji, formanty, układ, powiązanie danych i style.

## <a name="creating-the-application-project"></a><a name="Create_The_Application_Code_Files"></a> Tworzenie projektu aplikacji
...
4. W polu tekstowym **Nazwa** wprowadź wartość `ExpenseIt` , a następnie wybierz przycisk **OK** .
...
2. Jeśli używasz języka C#, Znajdź wiersz `<Window x:Class="ExpenseIt.MainWindow"` i zastąp go symbolem `<NavigationWindow x:Class="ExpenseIt.MainWindow"` .

     Jeśli używasz Visual Basic, Znajdź wiersz `<Window x:Class=" MainWindow"` i zastąp go `<NavigationWindow x:Class="MainWindow"` .

     Należy zauważyć, że po zmianie `<Window` znacznika na funkcja `<NavigationWindow` IntelliSense automatycznie zmienia tag zamykający `</NavigationWindow>` .
...
3. Wybierz `<Grid>` znaczniki i `</Grid>` Usuń je.
...
5. W oknie XAML ustaw wartość elementu **Height** na `375` i ustaw wartość właściwości **Width** na `500` .
     ...
     Plik **MainWindow. XAML** powinien teraz wyglądać następująco w języku C#:
...
2. Znajdź wiersz `public partial class MainWindow : Window` i zastąp go znakiem `public partial class MainWindow : NavigationWindow` .

     Spowoduje to zmianę `MainWindow` klasy pochodnej `NavigationWindow` . W Visual Basic, dzieje się to automatycznie po zmianie okna w języku XAML, więc nie są wymagane żadne zmiany w kodzie.

## <a name="adding-files-to-the-application"></a><a name="add_files_to_the_application"></a> Dodawanie plików do aplikacji
...
4. Wybierz `<Title>` element i zmień tytuł na **ExpenseIt — Strona główna**.

     Plik **ExpenseItHome. XAML** powinien teraz wyglądać następująco w języku C#:
...
6. Znajdź element line `Title="ExpenseIt" Height="375" Width="500">` i Dodaj `Source="ExpenseItHome.xaml"` Właściwość.

     Spowoduje to ustawienie **ExpenseItHome. XAML** jako pierwszej strony otwartej podczas uruchamiania aplikacji. Plik **MainWindow. XAML** powinien teraz wyglądać następująco w języku C#:
    ...
    ```
     ...
     Tak jak w przypadku właściwości ustawionych wcześniej, można ustawić `Source` Właściwość w kategorii **różne** okna **Właściwości** .
...
4. Wybierz `<Title>` element i zmień tytuł na **ExpenseIt — wyświetlanie wydatków**.
...
## <a name="creating-the-user-interface"></a><a name="Add_Layout"></a> Tworzenie interfejsu użytkownika
...
1. Otwórz **ExpenseItHome. XAML** i wybierz `<Grid>` element.

2. W oknie **Właściwości** rozwiń węzeł Kategoria **układu** i ustaw wartości **marginesu** na `10` ,, `10` `0` , i `10` , które odpowiadają marginesom w lewo, w prawo, u góry i u dołu.

     Element `Margin="10,0,10,10"` jest dodawany do `<Grid>` elementu w kodzie XAML. Ponownie można wprowadzić te wartości bezpośrednio w kodzie XAML zamiast w oknie **Właściwości** z tym samym wynikiem.
...
2. Dodaj następujący kod XAML tuż nad `</Grid>` tagiem, aby utworzyć `Border` `ListBox` kontrolki, i `Button` .
...
3. W oknie dialogowym **Dodaj istniejący element** znajdź obraz **watermark.png** , który właśnie został dodany, wybierz go, a następnie wybierz przycisk **Dodaj** .

    > [!NOTE]
    > Może być konieczne rozwinięcie listy **typów plików** i wybranie opcji **pliki obrazów**.

4. Otwórz plik **ExpenseItHome. XAML** i Dodaj następujący kod XAML tuż nad `</Grid>` tagiem, aby utworzyć obraz tła:
...
4. Przenieś kontrolki do drugiej kolumny, ustawiając `Grid.Column` wartość na 1. Przenieś każdy formant w dół w dół, zwiększając każdą `Grid.Row` wartość o 1.
...
5. Tuż przed `<Border` dodaniem przez element następującego kodu XAML w celu wyświetlenia tytułu:
    ...
    ```
     ...
     Zawartość pliku **ExpenseItHome. XAML** powinna teraz wyglądać następująco w języku C#:
...
2. Wybierz `<Button` element i Dodaj następujący kod XAML bezpośrednio po elemencie **HorizontalAlignment = "Right"** : `Click="Button_Click"` .

     Spowoduje to dodanie programu obsługi zdarzeń dla zdarzenia przycisku `Click` . Kod elementu ** przycisku<** powinien teraz wyglądać następująco:
...
2. Dodaj następujący kod XAML między `<Grid>` `</Grid>` tagami i:
...
2. Dodaj następujący kod XAML między `<Application.Resources>` tagami i `</Application.Resources>` :
    ...
    - `listHeaderStyle`: Aby sformatować `Border` kontrolki nagłówka listy.
...
3. Otwórz **ExpenseItHome. XAML** i Zastąp wszystkie `<Grid>` elementy między `</Grid>` elementami i następującym XAML
    ...
    ```
     ...
     Właściwości, takie jak `VerticalAlignment` i definiujące `FontFamily` wygląd poszczególnych kontrolek, są usuwane i zastępowane przez zastosowanie stylów.

4. Otwórz **ExpenseReportPage. XAML** i Zastąp wszystkie `<Grid>` elementy między elementami KOŃCOWYmi `</Grid>` następującym XAML
    ...
    ```
     ...
     Spowoduje to dodanie stylów do `<Label>` `<Border>` elementów i.
...
2. Dodaj następujący kod XAML:
    ...
    ```
     ...
     Ten kod tworzy `XmlDataProvider` klasę, która zawiera dane dla każdej osoby. Zwykle zostanie to załadowane jako plik, ale dla uproszczenia dane są dodawane wewnętrznie.

3. W `<Grid.Resources>` elemencie Dodaj następujący kod XAML:
...
4. Zastąp istniejący `<ListBox>` element następującym XAML.
    ...
    ```
     ...
     Ten kod wiąże `ItemsSource` Właściwość ze `ListBox` źródłem danych i stosuje szablon danych jako `ItemTemplate` .
...
2. W języku C# Dodaj następujący Konstruktor do klasy **ExpenseReportPage** lub w Visual Basic Zastąp istniejącą klasę następującymi:
...
4. Zamień `Click` kod programu obsługi zdarzeń na następujący:
...
2. Zastąp kod XAML dla elementów **nazwy** i **działu** `<StackPanel` następującym:
...
4. Zastąp `<DataGrid>` element następującym:
...
## <a name="best-practices"></a><a name="Best_Practices"></a> Najlepsze rozwiązania
...
## <a name="whats-next"></a><a name="Whats_Next"></a> Co dalej
...
- [Przegląd powiązań danych](https://msdn.microsoft.com/library/ms752347\(v=vs.100\).aspx)
