...
- Aby zmienić marginesy elementu względem jego kontenera układu, kliknij obszar *definiowania układu marginesu* , który pojawia się wokół elementu w obszarze kompozycji, gdy element jest zaznaczony i znajduje się w kontenerze układu. Ilustracja przedstawiająca moduł definiowania układu marginesów zawiera temat [Tworzenie interfejsu użytkownika przy użyciu Projektant XAML](../designers/creating-a-ui-by-using-xaml-designer-in-visual-studio.md).
    ...
    > [!IMPORTANT]
    > Elementy umieszczane w niektórych kontenerach układów, takie jak <xref:Windows.UI.Xaml.Controls.Canvas> , nie mają modułu definiowania układu marginesu. Elementy umieszczane wewnątrz obszaru <xref:Windows.UI.Xaml.Controls.StackPanel> definiowania układu mają marginesy dla lewego i prawego marginesu lub górnego i dolnego marginesu, w zależności od orientacji `StackPanel` .
...
2. Kliknij prawym przyciskiem myszy wybrane elementy, wskaż polecenie **Grupuj do**, a następnie kliknij typ kontenera układu, w którym ma się znajdować Grupa.
    ...
    > [!TIP]
    > W przypadku wybrania opcji <xref:Windows.UI.Xaml.Controls.Viewbox> , <xref:Windows.UI.Xaml.Controls.Border> lub <xref:Windows.UI.Xaml.Controls.ScrollViewer> grupowania elementów elementy są umieszczane w nowym <xref:Windows.UI.Xaml.Controls.Grid> panelu w <xref:Windows.UI.Xaml.Controls.Viewbox> , <xref:Windows.UI.Xaml.Controls.Border> lub <xref:Windows.UI.Xaml.Controls.ScrollViewer> . Jeśli rozgrupujesz elementy w jednym z tych kontenerów układu, tylko <xref:Windows.UI.Xaml.Controls.Viewbox> , <xref:Windows.UI.Xaml.Controls.Border> , lub <xref:Windows.UI.Xaml.Controls.ScrollViewer> zostanie usunięte, a <xref:Windows.UI.Xaml.Controls.Grid> panel pozostanie. Aby usunąć `Grid` panel, Rozgrupuj elementy ponownie.
...
- W oknie Konspekt dokumentu lub obszarze kompozycji kliknij element prawym przyciskiem myszy, wybierz polecenie **Układ**, **Zresetuj** *PropertyName*, gdzie *PropertyName* jest właściwością, która ma zostać zresetowana (lub wybierz pozycję **Układ**, **Zresetuj wszystkie** , aby zresetować wszystkie właściwości układu dla elementu).
