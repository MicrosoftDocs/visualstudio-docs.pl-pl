...
2. Wypełnij plik CSV następującymi danymi:
    ...
    |6|8|14|
     ...
     ![Wypełnij. Plik CSV z](../test/media/cuit-datadriven-adddatatocsvfile.png "CUIT_dataDriven_AddDataToCSVFile") danych
...
4. Plik CSV musi być skopiowany do katalogu wyjściowego lub test nie może zostać uruchomiony. Użyj okno Właściwości, aby je skopiować.
     ...
     ![Wdróż. CUIT_dataDriven_DeployCSVFile pliku CSV](../test/media/cuit-datadriven-deploycsvfile.png "")
...
1. Sprawdź, czy test jest teraz opartych na danych, ponownie uruchamiając test.
   ...
   **Wskazówki**
   ...
   Aby uzyskać dodatkowe informacje, [zobacz Testowanie ciągłego dostarczania przy użyciu programu Visual Studio 2012 — Rozdział 2: Testowanie jednostkowe: Testowanie wewnątrz](http://go.microsoft.com/fwlink/?LinkID=255188) i [testowanie w celu ciągłego dostarczania przy użyciu programu Visual Studio 2012 — Rozdział 5: Automatyzowanie testów systemowych](http://go.microsoft.com/fwlink/?LinkID=255196)
...
### <a name="q-can-i-use-data-driven-tests-on-my-windows-phone-app"></a>PYTANIA Czy można używać testów opartych na danych w mojej aplikacji Windows Phone?
 **Z** Tak. Kodowane testy interfejsu użytkownika oparte na danych dla Windows Phone są definiowane przy użyciu atrybutu DataRow dla metody testowej. W poniższym przykładzie x i y używają wartości 1 i 2 dla pierwszej iteracji i-1 i-2 dla drugiej iteracji testu.
 ...
 **Z** Wszelkie zmiany kodu wprowadzone w pliku UIMapDesigner.cs zostaną każdorazowo zastąpione przy generowaniu kodu za pomocą UIMap — Konstruktora kodowanego testu interfejsu użytkownika. W tym przykładzie i w większości przypadków zmiany kodu są konieczne, aby umożliwić testowi użycie źródła danych do pliku kodu źródłowego testu (czyli CodedUITest1.cs).
