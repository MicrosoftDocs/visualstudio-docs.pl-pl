...
- Testy jednostkowe można uruchomić w agencie kompilacji w ramach kompilacji.
  ...
  **Testowanie aplikacji programu SharePoint**
  ...
  Wymienione powyżej możliwości pozwalają również pisać testy jednostkowe i testy integracji dla aplikacji programu SharePoint przy użyciu programu Visual Studio. [!INCLUDE[crabout](../includes/crabout-md.md)] sposobu tworzenia aplikacji programu SharePoint przy użyciu programu Visual Studio, zobacz [Tworzenie rozwiązań SharePoint](https://msdn.microsoft.com/library/4bfb1e59-97c9-4594-93f8-3068b4eb9631), [Kompilowanie i debugowanie rozwiązań SharePoint](https://msdn.microsoft.com/library/c9e7c9ab-4eb3-40cd-a9b9-6c2a896f70ae) oraz [weryfikowanie i debugowanie kodu programu SharePoint](https://msdn.microsoft.com/library/b5f3bce2-6a51-41b1-a292-9e384bae420c).
...
6. Użyj listy rozwijanej **platformy docelowej (wszystkie konfiguracje)** , aby zmienić platformę docelową na **.NET Framework 3,5** lub nowszą, jak pokazano w objaśnieniu B na poniższej ilustracji. Nie należy określać wersji klienta.
     ...
     ![Lista rozwijana&#45;platformy docelowej](../test/media/howtoconfigureunitest35frameworkstepb.png "HowToConfigureUniTest35FrameworkStepB")

### <a name="re-targeting-to-a-specific-version-of-the-net-framework-for-visual-c-unit-test-projects"></a>Ponowne kierowanie do określonej wersji .NET Framework dla projektów testów C# jednostkowych

1. Utwórz nowy projekt Visual C# Unit-Test. W menu **plik** wybierz polecenie **Nowy** , a następnie wybierz **projekt**.

     Zostanie wyświetlone okno dialogowe **Nowy projekt** .

2. W obszarze **zainstalowane szablony**rozwiń **pozycję C#Visual** . Wybierz pozycję **test** , a następnie wybierz szablon **projektu testowego** .

3. W polu tekstowym **Nazwa** wpisz nazwę projektu testowego wizualizacji C# , a następnie wybierz **OK**.

4. W Eksplorator rozwiązań wybierz pozycję **Właściwości** z menu skrótów nowego projektu testu wizualnego C# .

     Zostaną wyświetlone właściwości projektu testowego wizualizacji C# .
...
5. Na karcie **aplikacja** wybierz **platformę docelową** , a następnie wybierz **.NET Framework 3,5** lub nowszą wersję z listy rozwijanej, aby zmienić element docelowy Framework.as pokazany na poniższej ilustracji. Nie należy określać wersji klienta.
     ...
     ![Lista rozwijana&#45;platformy docelowej](../test/media/howtoconfigureunittest35frameworkcsharp.png "HowToConfigureUnitTest35FrameworkCSharp")

### <a name="re-targeting-to-a-specific-version-of-the-net-framework-for-ccli-unit-test-projects"></a>Ponowne kierowanie do określonej wersji .NET Framework dla C++projektów testów jednostkowych/CLI

1. Utwórz nowy C++ projekt jednostki testowej. W menu **plik** wybierz pozycję **Nowy** , a następnie kliknij pozycję **projekt**.

     Zostanie wyświetlone okno dialogowe **Nowy projekt** .

    > [!WARNING]
    > Aby skompilować C++testy jednostkowe/CLI dla poprzedniej wersji programu .NET Framework dla wizualizacji C++, musisz użyć odpowiedniej wersji programu Visual Studio. Na przykład aby wskazać .NET Framework 3,5, należy zainstalować [!INCLUDE[vs_orcas_long](../includes/vs-orcas-long-md.md)] i [!INCLUDE[vs_orcas_long](../includes/vs-orcas-long-md.md)] dodatek Service Pack 1.
...
3. W polu tekstowym **Nazwa** wpisz nazwę projektu testowego wizualizacji C++ , a następnie kliknij przycisk **OK**.

4. W Eksplorator rozwiązań wybierz pozycję **Zwolnij projekt** z nowego projektu C++ testowego wizualizacji.

5. W Eksplorator rozwiązań wybierz niezaładowany projekt testu wizualizacji C++ , a następnie wybierz **edytuj \<project nazwa >. vcxproj**.

     Plik. vcxproj zostanie otwarty w edytorze.

6. Ustaw `TargetFrameworkVersion` w wersji 3,5 lub nowszej w `PropertyGroup` oznaczonej `"Globals"`. Nie należy określać wersji klienta:
...
8. W Eksplorator rozwiązań wybierz opcję Wybierz **Załaduj ponownie projekt** z menu skrótów nowego projektu testu C++ wizualnego.
