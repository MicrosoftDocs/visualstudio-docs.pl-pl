---
title: Testowanie jednostkowe C# kodu wizualizacji w aplikacji ze sklepu | Microsoft Docs
ms.date: 11/15/2016
...
## <a name="BKMK_Create_the_solution_and_the_unit_test_project"></a>Utwórz rozwiązanie i projekt testu jednostkowego

1. W menu **plik** wybierz pozycję **Nowy**, a następnie wybierz pozycję **Nowy projekt**.

2. W oknie dialogowym **Nowy projekt** rozwiń węzeł **zainstalowane**, rozwiń pozycję **Wizualizacja C#**  i wybierz pozycję **Sklep Windows**. Następnie wybierz **pustą aplikację** z listy szablonów projektu.

3. Nadaj projektowi nazwę `Maths` i upewnij się, że wybrano **Utwórz katalog dla rozwiązania** .
...
5. W oknie dialogowym **Nowy projekt** rozwiń węzeł **zainstalowane**, rozwiń pozycję **Wizualizacja C#**  i wybierz pozycję **Sklep Windows** . Następnie wybierz pozycję **Biblioteka testów jednostkowych (aplikacje ze sklepu Windows)** z listy szablonów projektu.
...
6. Otwórz UnitTest1.cs w edytorze programu Visual Studio.
    ...
    2. Metody testowe muszą znajdować się w klasie z atrybutem `[TestClass]`.
...
## <a name="BKMK_Verify_that_the_tests_run_in_Test_Explorer"></a>Sprawdź, czy testy są uruchamiane w Eksploratorze testów
...
## <a name="BKMK_Add_the_Rooter_class_to_the_Maths_project"></a>Dodawanie klasy Rooter do projektu Maths
...
3. Dodaj następujący kod do pliku **Rooter.Cser** klasy root:
    ...
    ```
     ...
     Klasa `Rooter` deklaruje konstruktora i metodę `SqareRoot` szacowania.

4. Metoda `SqareRoot` jest tylko minimalną implementacją, wystarczy do przetestowania podstawowej struktury instalacji testowej.

## <a name="BKMK_Couple_the_test_project_to_the_app_project"></a>Połącz projekt testowy z projektem aplikacji
...
2. Dodaj instrukcję using do pliku UnitTest1.cs:
   ...
   2. Dodaj następujący kod poniżej wiersza `using Microsoft.VisualStudio.TestPlatform.UnitTestFramework;`:
...
## <a name="BKMK_Iteratively_augment_the_tests_and_make_them_pass"></a>Iteracyjnie rozszerza testy i przekazują je
...
4. Podnieś poziom testowanego kodu, tak aby nowe testy zostały przekazane. Zmień funkcję `SqareRoot` w **Rooter.cs** na:
...
## <a name="BKMK_Debug_a_failing_test"></a>Debuguj test zakończony niepowodzeniem
 ...
 **Refaktoryzacja kodu testowego w celu wyeliminowania zduplikowanego kodu.**
 ...
 Należy zauważyć, że metoda `RangeTest` oznacza mianownik zmiennej tolerancji używanej w metodzie `Assert`. Jeśli planujesz dodać dodatkowe testy, które używają tego samego obliczenia tolerancji, użycie zakodowanej wartości w wielu lokalizacjach może prowadzić do błędów.
...
> [!NOTE]
> Aby dodać metodę pomocnika do klasy testowej, nie należy dodawać do metody atrybutu `[TestMethod]`. Eksplorator testów nie rejestruje metody do uruchomienia.
