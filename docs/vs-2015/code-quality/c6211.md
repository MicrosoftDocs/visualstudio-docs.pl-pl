---
title: C6211 | Microsoft Docs
ms.date: 11/15/2016
ms.prod: visual-studio-dev14
ms.technology: vs-ide-code-analysis
ms.topic: reference
f1_keywords:
- C6211
helpviewer_keywords:
- C6211
ms.assetid: 9b68243b-534c-4a05-b789-bb155dfcba1e
caps.latest.revision: 26
author: corob-msft
ms.author: corob
manager: jillfra
ms.openlocfilehash: 90b9829f05ac8c1eec2a422293b7121ad14035d5
ms.sourcegitcommit: 68f893f6e472df46f323db34a13a7034dccad25a
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 02/15/2020
ms.locfileid: "77277908"
---
# <a name="c6211"></a>C6211
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Ostrzeżenie C6211: przeciek > \<j pamięci z powodu wyjątku. Rozważ użycie lokalnego bloku catch do wyczyszczenia pamięci  
  
 To ostrzeżenie wskazuje, że przydzieloną pamięć nie zostanie zwolniona, gdy zostanie zgłoszony wyjątek. Instrukcja na końcu ścieżki może zgłosić wyjątek. Analizator sprawdza pod kątem tego warunku tylko wtedy, gdy została określona `_Analysis_mode_(_Analysis_local_leak_checks_)`j adnotacji SAL. Domyślnie ta adnotacja jest określona dla kodu trybu jądra systemu Windows (sterownika). Aby uzyskać więcej informacji na temat adnotacji SAL, zobacz [Używanie adnotacji salC++ w celu zmniejszenia wad języka C/Code](../code-quality/using-sal-annotations-to-reduce-c-cpp-code-defects.md).  
  
## <a name="example"></a>Przykład  
 Poniższy kod generuje to ostrzeżenie, ponieważ wyjątek może być zgłaszany podczas drugiego przydziału, a tym samym wyciekiem pierwszej alokacji lub wyjątek może zostać zgłoszony w kodzie, który jest reprezentowany przez komentarz "`code ...`", a tym samym wyciekiem obu alokacji.  
  
```cpp  
// cl.exe /analyze /c /EHsc /nologo /W4   
#include <sal.h>  
  
_Analysis_mode_(_Analysis_local_leak_checks_)   
void f( )  
{  
    char *p1 = new char[10];  
    char *p2 = new char[10];  
  
    // code ...  
  
    delete[] p2;  
    delete[] p1;  
}  
  
```  
  
 Aby użyć tych samych metod alokacji i rozwiązać ten problem, Dodaj program obsługi wyjątków:  
  
```cpp  
// cl.exe /analyze /c /EHsc /nologo /W4  
#include <sal.h>  
#include <new>  
#include <iostream>  
using namespace std;  
  
_Analysis_mode_(_Analysis_local_leak_checks_)   
  
void f()  
{  
    char *p1 = nullptr;   
    char *p2 = nullptr;  
  
    try  
    {  
        p1 = new char[10];  
        p2 = new char[10];  
  
        // code ...  
  
        delete [] p2;  
        delete [] p1;  
    }  
    catch (const bad_alloc& ba)  
    {  
        cout << ba.what() << endl;  
        delete [] p2;  
        delete [] p1;  
    }  
    // code ...  
}  
  
```  
  
 Aby całkowicie uniknąć tego rodzaju potencjalnych przecieków, użyj mechanizmów, które są dostarczane przez C++ standardową bibliotekę szablonów (STL). Należą do nich [shared_ptr](https://msdn.microsoft.com/library/1469fc51-c658-43f1-886c-f4530dd84860), [unique_ptr](https://msdn.microsoft.com/library/acdf046b-831e-4a4a-83aa-6d4ee467db9a)i [wektor](https://msdn.microsoft.com/library/c1431ad8-c0b6-4dbb-89c4-5f651e432d7f). Aby uzyskać więcej informacji, zobacz [inteligentne wskaźniki](https://msdn.microsoft.com/library/909ef870-904c-49b6-b8cd-e9d0b7dc9435) i [ C++ standardowa biblioteka](https://msdn.microsoft.com/library/a37d3ba3-58af-47c7-9ee2-441ccd7b77ee).  
  
```cpp  
// cl.exe /analyze /c /EHsc /nologo /W4  
#include <sal.h>  
#include <vector>  
#include <memory>  
  
using namespace std;  
  
_Analysis_mode_(_Analysis_local_leak_checks_)   
  
void f( )  
{  
    // use 10-element vectors in place of char[10]  
    vector<char> v1;  
    vector<char> v2;  
  
    for (int i=0; i<10; ++i) {  
        v1.push_back('a');  
        v2.push_back('b');  
    }  
    // code ...  
  
    // use unique_ptr if you still want char[10]  
    unique_ptr<char[]> a1(new char[10]);  
    unique_ptr<char[]> a2(new char[10]);  
  
    // code ...  
  
    // No need for delete; vector and unique_ptr   
    // clean up when out of scope.  
}  
  
```  
  
## <a name="see-also"></a>Zobacz też  
 [Obsługa wyjątków języka C++](https://msdn.microsoft.com/library/65f80b44-9d0f-4d17-b910-07205a5c5c40)
