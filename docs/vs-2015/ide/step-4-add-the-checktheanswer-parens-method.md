...
1. Dodaj metodę `CheckTheAnswer()`.
     ...
     Gdy ta metoda jest wywoływana, dodaje wartości addend1 i addend2 i porównuje wynik do wartości w kontrolce sumy `NumericUpDown`. Jeśli wartości są równe, metoda zwraca wartość `true`. W przeciwnym razie metoda zwraca wartość `false`. Kod powinien wyglądać podobnie do poniższego.
     ...
     Następnie sprawdź odpowiedź, aktualizując kod w metodzie dla programu obsługi zdarzeń taktu czasomierza, aby wywołać nową metodę `CheckTheAnswer()`.
