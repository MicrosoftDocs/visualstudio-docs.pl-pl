...
## <a name="a-new-setup-experience"></a>Nowe środowisko instalacji
 ...
 Środowisko instalacyjne programu Visual Studio 2015 zostało zgłoszone w celu zainstalowania tylko potrzebnych części. Dzięki temu instalacja jest szybsza dla wielu typowych scenariuszy obejmujących platformę .NET lub programowanie w sieci Web. W przypadku innych typów programowania, takich jak programowanie aplikacji mobilnych na wiele platform lub praca w języku C++ lub F #, wybierz opcję Instalacja **niestandardowa** , a następnie wybierz składniki i opcjonalne wymagane zestawy SDK innych firm. Możesz również zainstalować dowolny składnik niestandardowy później. Na przykład jeśli wybierzesz opcję Instalacja podstawowa, a następnie spróbujesz utworzyć nowy projekt w języku C++, zostanie wyświetlony monit o pobranie narzędzi programistycznych języka C++.
...
## <a name="sign-in-across-multiple-accounts"></a>Logowanie się na wielu kontach
 Dzięki programowi Visual Studio 2015 nowe usprawnione środowisko logowania ma znacznie uprościć dostęp do zasobów online nawet wtedy, gdy masz wiele kont programu Visual Studio. Po zalogowaniu się do programu Visual Studio użytkownik jest automatycznie zalogowany do wszystkich wystąpień programu Visual Studio 2015 i Blend na komputerze. Logowanie jest automatycznie uruchamiane w roamingu ustawień. W programie Visual Studio 2015 Twoje konto jest udostępniane w różnych funkcjach, tak długo, jak masz dobry token, możesz uzyskiwać dostęp do kont Visual Studio Team Services z **Team Explorer**i zasobów oraz witryn internetowych z subskrypcji Microsoft Azure w Eksplorator serwera. Zobaczysz również zasoby platformy Azure w oknie dialogowym Nowy projekt dla Application Insights projekty i zobaczysz konto [dewelopera](https://developer.salesforce.com/) Azure Mobile, Azure Storage, [Microsoft Office 365](https://msdn.microsoft.com/office/aa905340.aspx) i Saleforce.com w nowym oknie dialogowym **Dodawanie połączonej usługi** .
...
#### <a name="the-net-framework"></a>.NET Framework
 Firma Microsoft [!INCLUDE[net_v46](./includes/net-v46-md.md)] oferuje informacje o 150 nowych interfejsów API i zaktualizowanych interfejsach api 50, które umożliwiają obsługę większej liczby scenariuszy. Na przykład więcej kolekcji implementuje teraz, <xref:System.Collections.Generic.IReadOnlyCollection%601> ułatwiając ich użycie. Ponadto program ASP.NET 5, wymieniony wcześniej, oferuje platformę .NET Leaning do tworzenia nowoczesnych aplikacji opartych na chmurze.

 Aplikacje ze sklepu Windows zapisane w języku C#, które są przeznaczone dla .NET Framework mogą teraz korzystać z .NET Native, która kompiluje aplikacje do kodu natywnego, a nie IL, a [!INCLUDE[net_v46](./includes/net-v46-md.md)] także dodaje RyuJIT, 64-bitowy kompilator just-in-Time (JIT).

 Nowe kompilatory C# i VB ("Roslyn") znacząco przyspieszają czas kompilowania i zapewniają kompleksowe interfejsy API analizy kodu. Program Visual Studio 2015 wykorzystuje Roslyn, aby zapewnić więcej refaktoryzacji, takich jak wbudowane nazwy, analizatory i szybkie poprawki.
 ...
 Aby uzyskać więcej informacji, zobacz [co nowego](https://msdn.microsoft.com/library/1d971dd7-10fc-4692-8dac-30ca308fc0fa) i [blog platformy .NET](https://devblogs.microsoft.com/dotnet/).

#### <a name="c"></a>C++
 Visual C++ zapewnia znaczną zaawansowaność w języku C++ 11/14, obsługa tworzenia międzyplatformowych aplikacji mobilnych, obsługa funkcji możliwością wznowienia i oczekiwanie (obecnie planowanej normalizacji w języku C++ 17), ulepszenia i poprawki błędów w implementacjach biblioteki środowiska uruchomieniowego języka C (CRT) i C++ Standard Library (STL), okna dialogowe do zmiany rozmiaru w MFC, nowe optymalizacje kompilatora, lepsza wydajność kompilacji, nowe możliwości diagnostyki oraz nowe narzędzia produktywności w edytorze kodu.

 Aby uzyskać więcej informacji, zobacz [co nowego w Visual C++](https://msdn.microsoft.com/library/1cc09fad-85a2-43c2-b022-bb99f5fe0ad7) i [Visual C++ blog](https://devblogs.microsoft.com/cppblog/).
...
## <a name="cross-platform-debugging-support"></a>Obsługa debugowania dla wielu platform
 Możesz użyć programu Visual Studio do tworzenia i debugowania natywnych aplikacji mobilnych, które działają na urządzeniach z systemami Windows, iOS i Android. Używaj [emulatora programu Visual Studio dla systemu Android](https://devblogs.microsoft.com/devops/introducing-visual-studios-emulator-for-android/)lub podłącz urządzenie i Debuguj kod bezpośrednio w programie Visual Studio.
...
- **C#/Xamarin**. Użyj platformy [Xamarin](https://msdn.microsoft.com/library/dn879698\(v=vs.140\).aspx) do tworzenia natywnych aplikacji dla systemów Windows, iOS i Android w programie Visual Studio przy użyciu języka C#.

     [Debugowanie](/xamarin/ios/deploy-test/debugging-in-xamarin-ios?tabs=windows) (iOS) i [debugowanie na urządzeniu](/xamarin/android/deploy-test/debugging/debug-on-device?tabs=windows) w [przewodniku dewelopera platformy Xamarin](/xamarin/) opisuje środowisko debugowania.

- **C++/Android**. Używaj szablonów [Visual C++ dla opracowywania aplikacji mobilnych na wiele platform](cross-platform/visual-cpp-for-cross-platform-mobile-development.md) wraz z narzędziami innych firm, takimi jak zestaw [Android NDK](https://developer.android.com/tools/sdk/ndk/index.html) , do tworzenia natywnych aplikacji dla systemów Windows i Android.
...
### <a name="perftips"></a>Wskazówki dotyczące wydajności
 Funkcja PerfTip Wyświetlaj czas wykonywania metod podczas debugowania, co pozwala na szybkie wykrywanie wąskich gardeł bez konieczności wywoływania profilera. Aby rozpocząć, zobacz [Funkcja PerfTip: informacje o wydajności w skrócie, podczas debugowania w programie Visual Studio](https://devblogs.microsoft.com/devops/perftips-performance-information-at-a-glance-while-debugging-with-visual-studio/)

### <a name="error-list"></a>Lista błędów
 Lista błędów obsługuje teraz filtrowanie dla dowolnej kolumny. Przedstawia on również dynamiczny widok błędów, ostrzeżeń i analizy kodu w całym rozwiązaniu C# lub Visual Basic podczas pisania, nawet gdy zmiana kodu generuje tysiące ostrzeżeń. Nowy Lista błędów jest ponownie zgodny z istniejącym użyciem. Aby uzyskać więcej informacji, zobacz [okno Lista błędów](./ide/reference/error-list-window.md).

### <a name="gpu-usage-tool"></a>Narzędzie użycie procesora GPU
 Narzędzie użycie procesora GPU ułatwia zbieranie i analizowanie danych użycia procesora GPU w aplikacjach i grach DirectX oraz Rozwiązywanie problemów związanych z wąskimi gardłami wydajności w PROCESORAch lub procesorach GPU. Aby rozpocząć pracę z narzędziem, zapoznaj się z [wpisem na blogu Visual C++ zespołu](https://devblogs.microsoft.com/cppblog/gpu-usage-tool-in-visual-studio-2013-update-4-ctp1/).

## <a name="live-code-analysis-light-bulbs"></a>Analiza kodu na żywo (żarówki)
 Nowy kompilator Roslyn dla języka C# i Visual Basic nie tylko zapewnia krótszy czas kompilowania — umożliwia również korzystanie z całkowicie nowych scenariuszy, takich jak analiza kodu na żywo, które umożliwiają rozbudowaną i dostosowywalną opinię oraz sugestie bezpośrednio w edytorze kodu podczas wpisywania. W programie Visual Studio 2015 żarówki są wyświetlane na lewym marginesie (w przypadku korzystania z klawiatury) lub etykietki narzędzia (gdy wskaźnik myszy znajduje się nad błędem z myszą). Żarówka informuje w czasie rzeczywistym, że kompilator (prawdopodobnie korzystający z niestandardowego zestawu reguł) wykrył problem w kodzie i ma także sugestię dotyczącą sposobu rozwiązania problemu. Gdy zobaczysz żarówkę, kliknij ją w celu uzyskania sugestii do wykonania.
...
## <a name="visual-studio-extensibility-tools"></a>Visual Studio Extensibility Tools
 ...
 Jeśli chcesz dołączyć narzędzia rozszerzalności do instalacji niestandardowej, możesz je znaleźć w obszarze **funkcje/typowe narzędzia/Visual Studio Extensibility Tools**.  Możesz również zainstalować narzędzia rozszerzalności w późniejszym czasie, otwierając okno dialogowe **Nowy projekt** i wybierając pozycję **Zainstaluj Visual Studio Extensibility Tools** w obszarze **Visual C#/rozszerzalność**.
...
### <a name="send-a-smile"></a>Wyślij uśmiech
 ...
 Gotowe. Będziemy kierować swoją opinię do poprawnego zespołu, w którym będzie ona się oddzwonić, a następnie szybko zacząć zastanawiać się, jak jeszcze bardziej poprowadzisz.
...
### <a name="report-crashes-unresponsive-and-performance-issues"></a>Zgłaszaj awarie, niereagowanie i problemy z wydajnością
 Czasami szybka Uwaga w nieznanym niewoli jest zbyt mała, aby przekazać pełen wpływ niepotrzebnego elementu. Gdy program Visual Studio przestanie odpowiadać, ulegnie awarii lub wystąpi inny problem z wydajnością, można łatwo udostępnić kroki Odtwórz, Zrzuty awaryjne i pliki śledzenia przy użyciu okna dialogowego, które jest wyświetlane po wysłaniu niezadowolenia.
...
* [Twórz aplikacje mobilne dla wielu platform za pomocą Visual C++](./misc/build-cross-platform-mobile-apps-with-visual-cpp.md)
