...
1. Utwórz aplikację konsolową o nazwie `EncapsulateFieldExample` , a następnie zastąp ciąg `Program` następującym przykładowym kodem.
...
6. W przypadku wybrania opcji **Podgląd zmian odwołań** zostanie otwarte okno **zmiany w wersji zapoznawczej** . Następnie kliknij przycisk **Apply** (Zastosuj).

     Następujący `get` `set` kod dostępu jest wyświetlany w pliku źródłowym:
    ...
    ```
     ...
     Kod w `Main` metodzie jest również aktualizowany do nowej `Width` nazwy właściwości.
...
## <a name="see-also"></a>Zobacz też
 [Refaktoryzacje](../csharp-ide/refactoring-csharp.md) [kodu języka Visual c#](../ide/visual-csharp-code-snippets.md) (C#)