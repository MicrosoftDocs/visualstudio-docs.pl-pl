 ...
 **AdventureWorks** to bieżąca Przykładowa baza danych udostępniona dla SQL Server produktów. Możesz pobrać go jako plik. mdf ze [strony AdventureWorks w witrynie CodePlex](https://archive.codeplex.com/?p=msftdbprodsamples). W tym miejscu są dostępne regularne i uproszczone (LT) wersje bazy danych. W większości scenariuszy preferowana jest wersja LT, ponieważ jest ona mniej złożona.
