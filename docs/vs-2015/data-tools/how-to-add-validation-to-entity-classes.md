...
4. Projekty Visual Basic:
   ...
   5. Ponieważ projekty C# nie generują automatycznie programów obsługi zdarzeń, można użyć funkcji IntelliSense do tworzenia zmiennych metod częściowych.
...
4. Projekty Visual Basic:
   ...
   3. Metoda `Update` *ENTITYCLASSNAME* jest dodawana do klasy częściowej.
   ...
   5. Ponieważ projekty C# nie generują automatycznie obsługi zdarzeń, można użyć funkcji IntelliSense do utworzenia metody częściowej `Update` *CLASSNAME* .

   6. Wpisz `partial` , a następnie miejsce, aby uzyskać dostęp do listy dostępnych metod częściowych. Kliknij metodę Update dla klasy, dla której chcesz dodać sprawdzanie poprawności. Poniższy kod przypomina kod generowany podczas wybierania `Update` metody częściowej *CLASSNAME* :
