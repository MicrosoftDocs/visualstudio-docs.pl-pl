...
2. W oknie dialogowym **Nowy projekt** rozwiń węzeł **wizualizacji C#**  lub **Visual Basic** węzła, a następnie wybierz węzeł **systemu Windows** .
...
7. W polu **domyślny obszar nazw** (C#) lub **głównej przestrzeni nazw** ([!INCLUDE[vbprvb](../sharepoint/includes/vbprvb-md.md)]) wprowadź **ServerExplorer. SharePointConnections. WebPartNode**.
...
9. W polu **domyślny obszar nazw** (C#) lub **głównej przestrzeni nazw** ([!INCLUDE[vbprvb](../sharepoint/includes/vbprvb-md.md)]) wprowadź **ServerExplorer. SharePointConnections. WebPartNode**.
...
## <a name="define-a-node-type-that-represents-a-web-part"></a>Zdefiniuj typ węzła, który reprezentuje część sieci Web
 ...
 Aby zdefiniować nowy typ węzła, Klasa implementuje interfejs <xref:Microsoft.VisualStudio.SharePoint.Explorer.IExplorerNodeTypeProvider>. Zaimplementuj ten interfejs w każdym przypadku, gdy chcesz zdefiniować nowy typ węzła w **Eksplorator serwera**.
...
## <a name="create-the-custom-sharepoint-commands"></a>Tworzenie niestandardowych poleceń programu SharePoint
 Utwórz niestandardowe polecenia, które wywołują model obiektów serwera dla programu SharePoint, aby pobierać dane dotyczące składniki Web Part w witrynie programu SharePoint. Każde polecenie jest metodą, do której zastosowano <xref:Microsoft.VisualStudio.SharePoint.Commands.SharePointCommandAttribute>.
...
1. W eksperymentalnym wystąpieniu [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)], na pasku menu wybierz **widok** > **Eksplorator serwera**.
...
4. Sprawdź, czy kod w drugim wystąpieniu [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] jest zatrzymany w punkcie przerwania, który został ustawiony wcześniej w metodzie `NodeChildrenRequested`, a następnie naciśnij klawisz **F5** , aby kontynuować debugowanie projektu.
...
9. W wystąpieniu [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)], które chcesz debugować, sprawdź, czy szczegółowe informacje o składniku Web Part pojawiają się w oknie **Właściwości** .
...
1. W eksperymentalnym wystąpieniu [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]na pasku menu wybierz kolejno opcje **narzędzia** > **rozszerzenia i aktualizacje**.
