...
Zdarzenia przenośne są grupą liczników procesora, które nie są specyficzne dla określonego procesora. Wszystkie inne liczniki procesora CPU są nazywane zdarzeniami platformy i mogą nie być obsługiwane na różnych platformach.
 ...
 Liczniki dla zdarzeń przenośnych i platform są zdefiniowane w. pliki *XML* , gdzie określone wartości, które są związane z licznikami, są dostarczane. Istnieje wiele plików dla różnych procesorów CPU, ponieważ dane dla procesorów Intel i AMD, na przykład są różne. Profiler [!INCLUDE[vs_orcas_long](../debugger/includes/vs_orcas_long_md.md)] używa tych informacji, aby przedstawić odpowiednie liczniki, zarówno przenośne, jak i platformy, do użytkownika w celu pomiaru wydajności.
...
|Nazwa zdarzenia|Opis wydarzenia|
|----------------|-----------------------|
