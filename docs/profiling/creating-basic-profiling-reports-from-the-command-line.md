 ...
 **VSPerfReport** `VSPFile` **której** generuje wszystkie raporty dostępne dla. *Vsp* lub. *vsps* pliku.

 **VSPerfReport** `VSPFile` **/Summary:**`ReportType`[,`ReportType`...] Generuje typy określonego raportu.

 **VSPerfReport** `VSPFile` **/calltrace** generuje raport zawierający listę zdarzeń zbierania danych. Tylko Instrumentacja.
