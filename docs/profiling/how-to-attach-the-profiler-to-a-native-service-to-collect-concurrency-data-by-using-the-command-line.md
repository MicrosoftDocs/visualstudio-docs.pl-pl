---
title: 'VSPerfCmd: dołączanie profilera do usługi natywnej w celu uzyskania danych współbieżności'
ms.date: 11/04/2016
ms.topic: conceptual
...
W tym artykule opisano, jak używać narzędzi wiersza polecenia [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] narzędzia profilowania do dołączania profilera do natywnej usługi (CC++/) i zbierania danych współbieżności procesu i wątku przy użyciu metody próbkowania.
...
- Pary opcji w poniższej tabeli rozpoczynają i zatrzymują zbieranie danych. Określ każdą opcję w osobnym wierszu polecenia. Zbieranie danych można włączać i wyłączać wiele razy.
    ...
    |[/ProcessOn](../profiling/processon-and-processoff.md) **:** `PID` [/ProcessOff](../profiling/processon-and-processoff.md) **:** `PID`|Uruchamia ( **/ProcessOn**) lub zatrzymywanie ( **/ProcessOff**) zbieranie danych dla procesu, który określa identyfikator procesu (`PID`).|
