...
---
# <a name="how-to-attach-the-profiler-to-a-net-framework-stand-alone-application-and-collect-application-statistics-by-using-the-command-line"></a>Instrukcje: dołączanie profilera do .NET Framework aplikacji autonomicznej i zbieranie statystyk aplikacji przy użyciu wiersza polecenia
W tym artykule opisano sposób użycia narzędzi wiersza polecenia [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] narzędzia profilowania do dołączania profilera do uruchomionej .NET Framework aplikacji autonomicznej (klient) i zbierania statystyk wydajności przy użyciu metody próbkowania.
...
5. Dołącz profiler do aplikacji docelowej. Wpisz:
   ...
   - `PID` określa identyfikator procesu aplikacji docelowej. `ProcessName` określa nazwę procesu. Należy pamiętać, że jeśli określono `ProcessName` i wiele procesów, które mają taką samą nazwę, wyniki są nieprzewidywalne. Identyfikatory procesów wszystkich uruchomionych procesów można wyświetlić w Menedżerze zadań systemu Windows.
