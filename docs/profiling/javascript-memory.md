...
3. Na pasku menu wybierz kolejno opcje **debuguj** > narzędzie do **oceny wydajności**.
...
## <a name="show-built-in-objects"></a>Pokaż obiekty wbudowane
 ...
 Aby wyświetlić te obiekty, wybierz pozycję **Pokaż wbudowane** na ![liście rozwijanej&#45;ustawienia listy ustawienia w analizatorze pamięci](../profiling/media/js_mem_settings.png "JS_Mem_Settings") w prawym górnym rogu okienka.
...
- Postępuj zgodnie z przepływem pracy opisanym w [odizolowaniu przecieku pamięci](#isolate-a-memory-leak) i Użyj **obiektów z lewej strony od migawki #\<numer >** filtr w widoku diff, aby zidentyfikować podejrzane sugestie dotyczące przecieków pamięci.
...
- Poszukaj obiektów, które są przechowywane w pamięci przypadkowo po przejściu do nowej strony, która jest często przyczyną problemów z pamięcią. Na przykład:
  ...
  - Niektóre obiekty mogą zawierać metodę `dispose` i zalecenia dotyczące użycia. Na przykład, należy wywołać `dispose` na [WinJS. Binding. list](/previous-versions/windows/apps/hh700774\(v\=win.10\)) , jeśli wywołasz metodę `createFiltered` listy, a następnie opuścisz stronę.
...
- Odczytuj [Zarządzanie pamięcią w aplikacjach platformy UWP](https://msdn.microsoft.com/magazine/jj651575.aspx).
