...
Generuje manifest aplikacji ClickOnce lub natywny manifest. Manifest natywny opisuje składnik, definiując unikatową tożsamość dla składnika i identyfikując wszystkie zestawy i pliki tworzące składnik. Manifest aplikacji ClickOnce rozszerza natywny manifest przez wskazanie punktu wejścia aplikacji i określenie poziomu zabezpieczeń aplikacji.
...
| `EntryPoint` | Opcjonalny <xref:Microsoft.Build.Framework.ITaskItem> `[]` parametr.<br /><br /> Określa pojedynczy element, który wskazuje punkt wejścia dla wygenerowanego zestawu manifestu.<br /><br /> W przypadku manifestu aplikacji ClickOnce ten parametr określa zestaw, który jest uruchamiany, gdy aplikacja jest uruchamiana. |
| `ErrorReportUrl` | Opcjonalny <xref:System.String?displayProperty=fullName> parametr.<br /><br /> Określa adres URL strony sieci Web, która jest wyświetlana w oknach dialogowych podczas raportów o błędach w instalacjach technologii ClickOnce. |
...
| `HostInBrowser` | Opcjonalny <xref:System.Boolean> parametr.<br /><br /> Jeśli `true` aplikacja jest hostowana w przeglądarce (w przypadku aplikacji przeglądarki sieci Web WPF). |
...
| `TargetFrameworkMoniker` | Opcjonalny `String` parametr.<br /><br /> Określa moniker platformy docelowej. |
| `TargetFrameworkProfile` | Opcjonalny `String` parametr.<br /><br /> Określa Profil platformy docelowej. |
| `TargetFrameworkSubset` | Opcjonalny `String` parametr.<br /><br /> Określa nazwę podzbioru .NET Framework, który ma być obiektem docelowym. |
| `TargetFrameworkVersion` | Opcjonalny `String` parametr.<br /><br /> Określa .NET Framework docelowy projektu. |
...
| `UseApplicationTrust` | Opcjonalny `Boolean` parametr.<br /><br /> W przypadku wartości true `Product` `Publisher` właściwości, i `SupportUrl` są zapisywane w manifeście aplikacji. |
...
Ilustruje to najprostszy możliwy scenariusz generowania manifestu, w którym są generowane manifesty ClickOnce dla pojedynczego programu. Nazwa domyślna i tożsamość są wywnioskowane z zestawu dla manifestu.

> [!NOTE]
> W poniższym przykładzie wszystkie pliki binarne aplikacji są wstępnie skompilowane w celu skoncentrowania się na aspektach generowania manifestu. Ten przykład powoduje utworzenie w pełni działającego wdrożenia ClickOnce.
>
> [!NOTE]
