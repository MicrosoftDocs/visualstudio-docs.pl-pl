...
## <a name="c-project-vcxproj-and-solution-sln-files"></a>C++pliki projektu (. vcxproj) i rozwiązania (. sln)
 ...
 Zarówno C++ pliki projektów ( *. vcxproj*), jak i rozwiązania ( *. sln*) mogą być przesyłane do [zadania programu MSBuild](../msbuild/msbuild-task.md). W C++ przypadku projektów VCWrapperProject jest wywoływana, a następnie zostanie utworzony wewnętrzny projekt MSBuild. W C++ przypadku rozwiązań tworzony jest SolutionWrapperProject, a następnie tworzony jest wewnętrzny projekt MSBuild. W obu przypadkach projekt, który jest traktowany, jest traktowane tak samo jak każdy inny projekt MSBuild.
