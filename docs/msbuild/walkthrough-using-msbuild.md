...
2. W przeglądarce **Właściwości** zwróć uwagę, że właściwość **Plik projektu** to *BuildApp.csproj*. Wszystkie pliki projektu są nazwane z przyrostkiem *proj*. Jeśli utworzono projekt visual basic, nazwa pliku projektu będzie *BuildApp.vbproj*.

3. Kliknij prawym przyciskiem myszy węzeł projektu, a następnie kliknij polecenie **Zwolnij projekt**.

4. Kliknij prawym przyciskiem myszy węzeł projektu ponownie, a następnie kliknij polecenie **Edytuj plik BuildApp.csproj**.
...
W elemencie Projektu należy określić obszar nazw xmlns. Jeśli `ToolsVersion` jest obecny w nowym projekcie, musi to być "15.0".
...
> [!NOTE]
> Niektóre typy projektów, takie jak .NET Core, `Sdk` używają uproszczonego `ToolsVersion`schematu z atrybutem zamiast . Te projekty mają niejawne importy i różne domyślne wartości atrybutów.
...
Message Zadanie przyjmuje wartość ciągu text atrybutu jako dane wejściowe i wyświetla go na urządzeniu wyjściowym. Cel HelloWorld wykonuje zadanie Message dwa razy: najpierw wyświetlić "Hello", a następnie wyświetlić "Świat".
...
3. Uruchom msbuild z przełącznikiem polecenia -t:HelloWorld. Spowoduje to wybranie i zbudowanie celu HelloWorld:
...
```
 ...
 definiuje właściwość o nazwie TargetFrameworkVersion, nadając jej wartość ciągu "v15.0".
...
## <a name="examine-a-property-value"></a>Sprawdzanie wartości właściwości
 ...
 Aby uzyskać wartość właściwości, należy użyć następującej składni, gdzie Nazwa propertyname jest nazwą właściwości:
...
4. Zbadaj dane wyjściowe. Powinny być widoczne te dwa wiersze (twoja wersja programu .NET Framework może się różnić):
...
> [!NOTE]
> Jeśli nie widzisz tych wierszy, prawdopodobnie zapomniałeś zapisać plik projektu w edytorze kodu. Zapisz plik i spróbuj ponownie.

### <a name="conditional-properties"></a>Właściwości warunkowe

 Wiele właściwości, takich jak Konfiguracja są zdefiniowane warunkowo, oznacza to, że Condition atrybut pojawia się w elemencie właściwości. Właściwości warunkowe są definiowane lub ponownie definiowane tylko wtedy, gdy warunek ma wartość "true". Należy zauważyć, że niezdefiniowane właściwości są podane domyślną wartość pustego ciągu. Na przykład:
...
2. Zbadaj dane wyjściowe. Powinieneś zobaczyć ten wiersz:
...
> [!NOTE]
> Ścieżki plików są względem folderu zawierającego plik projektu MSBuild.
...
4. Zbadaj dane wyjściowe. Powinieneś zobaczyć tę długą linię:
