...
Aby otworzyć okno **Właściwości** , użyj menu kontekstowego (kliknij prawym przyciskiem myszy). Domyślnie okno **Właściwości** jest wyświetlane w prawym dolnym rogu programu Visual Studio. Po kliknięciu węzła, który jest renderowany w widoku modelu zawartości, właściwości tego węzła będą wyświetlane w oknie **Właściwości** .
...
Możesz przełączać się między widokiem wykresu i edytorem XML, klikając węzeł i korzystając z menu Wyświetl kontekst kodu (kliknij prawym przyciskiem myszy).
