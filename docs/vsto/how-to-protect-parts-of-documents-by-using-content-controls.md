...
## <a name="EditDeleteControl"></a>Ochrona kontrolki zawartości
 ...
 Możesz również chronić kontrolki zawartości dodawane do dokumentu w czasie wykonywania przy użyciu projektu dodatku VSTO. Aby uzyskać więcej informacji, zobacz [jak: Dodawanie kontrolek zawartości do dokumentów](../vsto/how-to-add-content-controls-to-word-documents.md)programu Word.
...
1. Ustaw właściwość kontrolki zawartość na **wartość true** `LockContentControl` , aby uniemożliwić użytkownikom edytowanie kontrolki i ustawić właściwość na true, aby uniemożliwić użytkownikom usuwanie kontrolek. `LockContents`

     Poniższy przykład kodu demonstruje użycie <xref:Microsoft.Office.Tools.Word.RichTextContentControl.LockContents%2A> właściwości i <xref:Microsoft.Office.Tools.Word.RichTextContentControl.LockContentControl%2A> dwóch różnych <xref:Microsoft.Office.Tools.Word.RichTextContentControl> obiektów w projekcie na poziomie dokumentu. Aby uruchomić ten kod, Dodaj kod do `ThisDocument` klasy w projekcie i `AddProtectedContentControls` Wywołaj metodę z `ThisDocument_Startup` procedury obsługi zdarzeń.
     ...
     Poniższy przykład kodu demonstruje użycie <xref:Microsoft.Office.Tools.Word.RichTextContentControl.LockContents%2A> właściwości i <xref:Microsoft.Office.Tools.Word.RichTextContentControl.LockContentControl%2A> dwóch różnych <xref:Microsoft.Office.Tools.Word.RichTextContentControl> obiektów w projekcie dodatku VSTO. Aby uruchomić ten kod, Dodaj kod do `ThisAddIn` klasy w projekcie i `AddProtectedContentControls` Wywołaj metodę z `ThisAddIn_Startup` procedury obsługi zdarzeń.
...
3. W grupie **formanty** kliknij przycisk listy rozwijanej **Grupa** , a następnie kliknij pozycję **Grupa**.
     ...
     Element <xref:Microsoft.Office.Tools.Word.GroupContentControl> zawierający chroniony region jest generowany automatycznie `ThisDocument` w klasie w projekcie. Obramowanie reprezentujące formant grupy jest widoczne w czasie projektowania, ale nie ma żadnych widocznych obramowania w czasie wykonywania.

### <a name="to-protect-an-area-of-a-document-at-run-time"></a>Aby chronić obszar dokumentu w czasie wykonywania

1. Program programowo zaznacz obszar, który ma być chroniony, a następnie Wywołaj <xref:Microsoft.Office.Tools.Word.ControlCollection.AddGroupContentControl%2A> metodę, aby <xref:Microsoft.Office.Tools.Word.GroupContentControl>utworzyć.

     Poniższy przykład kodu dla projektu na poziomie dokumentu dodaje tekst do pierwszego akapitu w dokumencie, wybiera pierwszy akapit, a następnie tworzy wystąpienie a <xref:Microsoft.Office.Tools.Word.GroupContentControl>. Aby uruchomić ten kod, Dodaj kod do `ThisDocument` klasy w projekcie i `ProtectFirstParagraph` Wywołaj metodę z `ThisDocument_Startup` procedury obsługi zdarzeń.
     ...
     Poniższy przykład kodu dla projektu dodatku VSTO dodaje tekst do pierwszego akapitu w aktywnym dokumencie, wybiera pierwszy akapit, a następnie tworzy wystąpienie a <xref:Microsoft.Office.Tools.Word.GroupContentControl>. Aby uruchomić ten kod, Dodaj kod do `ThisAddIn` klasy w projekcie i `ProtectFirstParagraph` Wywołaj metodę z `ThisAddIn_Startup` procedury obsługi zdarzeń.
