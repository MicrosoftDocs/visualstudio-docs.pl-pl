...
- [Wycofywanie rozwiązania do wcześniejszej wersji](#Roll)
  ...
  Aby uzyskać więcej informacji na temat wdrażania rozwiązania pakietu Office przez utworzenie pliku Instalator Windows, zobacz [wdrażanie rozwiązania biurowego przy użyciu Instalator Windows](../vsto/deploying-an-office-solution-by-using-windows-installer.md).

## <a name="Publish"></a>Publikowanie rozwiązania
 Rozwiązanie można opublikować za pomocą **Kreatora publikacji** lub **projektanta projektu**. W ramach tej procedury będziesz używać **projektanta projektu** , ponieważ zawiera kompletny zestaw opcji publikacji. Zobacz [Kreator &#40;publikowania — Programowanie Office w programie&#41;Visual Studio](../vsto/publish-wizard-office-development-in-visual-studio.md).
...
## <a name="Trust"></a>Zdecyduj, w jaki sposób chcesz udzielić zaufania do rozwiązania
...
## <a name="Helping"></a>Pomóż użytkownikom zainstalować rozwiązanie
...
1. Na serwerze, na którym działa program IIS 6,0, wybierz kolejno opcje **Start** > **Wszystkie programy** > **Narzędzia administracyjne** >  **Internet Information Services (IIS) Manager**.

2. Wybierz nazwę komputera, folder **witryny sieci Web** lub witrynę sieci Web, którą konfigurujesz.

3. Na pasku menu wybierz **akcja** > **Właściwości**.
...
1. Na serwerze, na którym działa program IIS 7,0, wybierz kolejno opcje **Start** > **Wszystkie programy** > **akcesoria**.
...
6. Na pasku menu wybierz **projekt** > **Dodaj odwołanie**.
...
8. Zmień nazwę klasy na `FileCopyPDA`, a następnie zastąp zawartość pliku kodem. Kod będzie wykonywał następujące zadania:

   - Kopiowanie dokumentu do komputera użytkownika.

   - Zmienia właściwość _AssemblyLocation ze ścieżki względnej na w pełni kwalifikowaną ścieżkę do manifestu wdrożenia.
...
4. Po elemencie `</vstav3:update>` Dodaj następujący kod. Dla atrybutu Class elementu `<vstav3:entryPoint>` należy użyć następującej składni: *NamespaceName. ClassName*. W poniższym przykładzie nazwy przestrzeni nazw i klas są takie same, więc nazwa punktu wejścia jest `FileCopyPDA.FileCopyPDA`.
...
1. W folderze **%USERPROFILE%\Documents\Visual Studio 2013 \ Projects\ExcelWorkbook\ExcelWorkbook** Skopiuj plik certyfikatu **ExcelWorkbook_TemporaryKey. pfx** , a następnie wklej go do plików *PublishFolder* **\Dane \ ExcelWorkbook**\__MostRecentPublishedVersion_ .

2. Otwórz wiersz polecenia programu Visual Studio, a następnie zmień katalogi na folder **C:\publish\Application Files\ExcelWorkbook**\__MostRecentPublishedVersion_ (na przykład **c:\publish\Application Files\ExcelWorkbook_1_0_0 _4**).
...
5. Skopiuj plik *ExcelWorkbook. VSTO* do katalogu **C:\publish\Application Files\ExcelWorkbook**\__MostRecentVersionNumber_ .

## <a name="SharePoint"></a>Umieszczanie dokumentu rozwiązania na serwerze, na którym jest uruchomiony program SharePoint (tylko dostosowania na poziomie dokumentu)
 ...
 *%commonprogramfiles%\microsoft shared\VSTO\10.0\VSTOInstaller.exe*
 ...
 Jeśli narzędzie nie znajduje się w tej lokalizacji, można użyć rejestru **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\VSTO Runtime Setup\v4\InstallerPath** lub **HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\VSTO Runtime Setup\v4\InstallerPath** klucz, aby znaleźć ścieżkę do tego narzędzia.

 Można użyć następujących parametrów z *VSTOInstaller. exe*.
...
| /Help lub /? | Wyświetlanie informacji Pomocy. |
 ...
 Po uruchomieniu programu *VSTOInstaller. exe*mogą pojawić się następujące kody błędów.
...
2. Uruchom program instalacyjny i Dołącz `/url` parametr, który przyjmuje nową ścieżkę instalacji jako ciąg.
   ...
   > [!NOTE]
   > Aby wyświetlić bieżącą wartość adresu URL, uruchom `setup.exe /url`.
...
4. Przed identyfikatorem wprowadź w pełni kwalifikowaną ścieżkę do dokumentu, a po nim pasek w *ścieżce* formatu|*Identyfikator* (na przykład *File://servername/FolderName/filename|74744e4b-e4d6-41eb-84f7-ad20346fe2d9*.
