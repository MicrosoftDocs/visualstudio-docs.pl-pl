---
title: Aktualizowanie dostosowań Wstążki migrowanych do .NET Framework 4,5
description: Dowiedz się, że musisz wprowadzić zmiany w kodzie projektu, jeśli struktura docelowa zostanie zmieniona na .NET Framework 4 lub nowszy.
...
ms.locfileid: "99838311"
---
# <a name="update-ribbon-customizations-migrated-to-net-framework-45"></a>Aktualizowanie dostosowań Wstążki migrowanych do .NET Framework 4,5
...
- W projektach Visual Basic lub w projektach Visual C#, które zostały utworzone w [!INCLUDE[vs_dev11_long](../sharepoint/includes/vs-dev11-long-md.md)] lub [!INCLUDE[vs_dev10_long](../sharepoint/includes/vs-dev10-long-md.md)] wykonaj wszystkie kroki w pliku związanym z kodem wstążki (*YourRibbonItem*. Designer.cs lub *YourRibbonItem*. Designer. vb). Aby wyświetlić plik związany z kodem w projektach Visual Basic, kliknij przycisk **Pokaż wszystkie pliki** w **Eksplorator rozwiązań**.

- W projektach Visual C# utworzonych w programie Visual Studio 2008, a następnie uaktualnioniu do [!INCLUDE[vs_dev12](../vsto/includes/vs-dev12-md.md)] , wykonaj dwa pierwsze kroki w pliku kodu wstążki (*YourRibbonItem*. cs lub *YourRibbonItem*. vb), a następnie wykonaj pozostałe kroki w pliku związanym z kodem wstążki.

### <a name="to-change-the-generated-ribbon-code"></a>Aby zmienić wygenerowany kod wstążki

1. Zmodyfikuj deklarację klasy wstążki, aby dziedziczyć ją od <xref:Microsoft.Office.Tools.Ribbon.RibbonBase> zamiast `Microsoft.Office.Tools.Ribbon.OfficeRibbon` .
...
3. W `InitializeComponent` metodzie zmodyfikuj każdy kod, który konstruuje kontrolkę wstążki, tak aby kod używał jednej z metod pomocnika <xref:Microsoft.Office.Tools.Ribbon.RibbonFactory> obiektu.

    > [!NOTE]
    > W projektach Visual C#, należy rozwinąć region o nazwie, `Component Designer generated code` Aby zobaczyć `InitializeComponent` metodę.
    ...
    ```
     ...
     W projekcie, który jest [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] przeznaczony dla lub później, należy zamiast tego użyć poniższego kodu.
...
4. W projektach w języku Visual C# zmodyfikuj dowolny wiersz kodu w `InitializeComponent` metodzie, która używa <xref:System.EventHandler%601> delegata, aby zamiast tego użyć określonego delegata wstążki.
    ...
    \<CodeContentPlaceHolder>8 </CodeContentPlaceHolder> w projekcie, który jest [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] przeznaczony dla lub później, należy zamiast tego użyć poniższego kodu.
...
## <a name="instantiate-ribbon-controls"></a><a name="ribboncontrols"></a> Tworzenie wystąpień kontrolek wstążki
 Należy zmodyfikować każdy kod, który dynamicznie tworzy wystąpienia kontrolek wstążki. W projektach, które są przeznaczone dla .NET Framework 3,5, formanty wstążki są klasy, które można utworzyć bezpośrednio w niektórych scenariuszach. W projektach, które są przeznaczone dla [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] lub później, te kontrolki są interfejsami, których nie można utworzyć bezpośrednio. Należy utworzyć kontrolki przy użyciu metod dostarczonych przez <xref:Microsoft.Office.Tools.Ribbon.RibbonFactory> obiekt.

 Istnieją dwa sposoby uzyskania dostępu do <xref:Microsoft.Office.Tools.Ribbon.RibbonFactory> obiektu:
...
\<CodeContentPlaceHolder>10 </CodeContentPlaceHolder> 
 \<CodeContentPlaceHolder> 11 </CodeContentPlaceHolder> Poniższa tabela zawiera listę formantów, które można tworzyć programowo i metodę, która ma być używana do tworzenia kontrolek w projektach przeznaczonych dla [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] lub w późniejszym czasie.

|Kontrola|Metoda RibbonFactory do użycia w [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] projektach i nowszych|
|-------------| - |
...
## <a name="handle-ribbon-events"></a><a name="ribbonevents"></a> Obsłuż zdarzenia wstążki
 Należy zmodyfikować dowolny kod, który obsługuje zdarzenia formantów wstążki. W projektach przeznaczonych dla .NET Framework 3,5 te zdarzenia są obsługiwane przez <xref:System.EventHandler%601> delegata ogólnego. W projektach przeznaczonych dla [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] lub nowszych zdarzeń te zdarzenia są teraz obsługiwane przez innych delegatów.
...
## <a name="set-the-position-of-a-ribbon-component-programmatically"></a>Ustaw położenie składnika wstążki programowo
 Należy zmodyfikować każdy kod, który ustawia położenie grup wstążki, kart lub kontrolek. W projektach przeznaczonych dla .NET Framework 3,5 można użyć `AfterOfficeId` `BeforeOfficeId` metod i klasy statycznej, `Microsoft.Office.Tools.Ribbon.RibbonPosition` Aby przypisać `Position` Właściwość grupy, karty lub kontrolki. W projektach przeznaczonych dla [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] lub nowszych należy uzyskać dostęp do tych metod przy użyciu <xref:Microsoft.Office.Tools.Ribbon.RibbonFactory.RibbonPosition%2A> Właściwości dostarczonej przez <xref:Microsoft.Office.Tools.Ribbon.RibbonFactory> obiekt.
...
```
 ...
 Poniższy przykład kodu demonstruje to samo zadanie w projekcie, który odwołuje się do [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] .
