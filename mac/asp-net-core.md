...
Otwórz program Visual Studio dla komputerów Mac. Na ekranie startowym wybierz pozycję **Nowy projekt...**
...
- **> rdzenia .NET puste**
- **Interfejs API > rdzenia platformy .NET**
- **Aplikacja sieciowa > sieci Web**
- **.NET Core > aplikacji sieci Web (model-view-controller)**

![ASP.NET opcje projektu](media/asp-net-core-2019-new-asp-core.png)

Wybierz **ASP.NET Core Empty Web Application** i naciśnij przycisk **Dalej**. Nadaj projektowi nazwę i naciśnij klawisz **Utwórz**. Spowoduje to utworzenie nowej aplikacji ASP.NET Core. W lewym okienku pola rozwiązania rozwiń drugą strzałkę, a następnie wybierz **Startup.cs**. Powinien wyglądać podobnie do poniższego obrazu:

![Nowy widok pustego projektu ASP.NET core](media/asp-net-core-2019-empty-project.png)

Szablon ASP.NET Core Empty tworzy aplikację internetową z dwoma plikami domyślnymi: **Program.cs** i **Startup.cs**, które zostały wyjaśnione poniżej. Tworzy również folder zależności, który zawiera zależności pakietu NuGet projektu, takie jak ASP.NET Core, .NET Core framework i MSBuild obiektów docelowych, które tworzą projekt:

![Klawiatura rozdzielająca z zależnościami](media/asp-net-core-2019-solution-dependencies.png)

### <a name="programcs"></a>Program.cs

Otwórz i sprawdź **plik Program.cs** w projekcie. Należy zauważyć, że kilka `Main` rzeczy dzieje się w metodzie — wpis do aplikacji:
...
Startup Klasy dla aplikacji jest `UseStartup()` określony w `CreateWebHostBuilder`metodzie na . Jest w tej klasie, która określi potok obsługi żądań i gdzie można skonfigurować żadnych usług.
...
Ten prosty projekt Hello, World można uruchomić teraz bez dodatkowego kodu są dodawane. Aby uruchomić aplikację, możesz wybrać przeglądarkę, w której chcesz uruchomić aplikację, korzystając z listy rozwijanej po prawej stronie przycisku Odtwórz, lub po prostu nacisnąć przycisk Odtwórz (trójkątny), aby użyć domyślnej przeglądarki:

![Uruchom przeglądarkę](media/asp-net-web-picker.png)
...
Po uruchomieniu projektu domyślna przeglądarka internetowa powinna uruchomić i połączyć się z adresem URL wymienionym w danych wyjściowych aplikacji. Alternatywnie można otworzyć dowolną przeglądarkę i `http://localhost:5000/`wprowadzić , `5000` zastępując portem, który visual studio danych wyjściowych w danych wyjściowych aplikacji. Powinien zostać wyświetlony `Hello World!`tekst:
...
Jeśli chcesz ręcznie zainstalować program .NET Core w systemie Mac OS 10.12 (Sierra) lub nowszym, wykonaj następujące czynności:
...
Upewnij się, że wszystkie kroki zostały pomyślnie zakończone, aby upewnić się, że program .NET Core został pomyślnie zainstalowany.
