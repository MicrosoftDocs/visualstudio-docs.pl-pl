...
ms.locfileid: "75405819"
---
# <a name="visual-studio-2019-for-mac-tour"></a>Przewodnik po programie Visual Studio 2019 dla komputerów Mac

Visual Studio dla komputerów Mac to _zintegrowane środowisko programistyczne_ platformy .NET na komputerze Mac, które może służyć do edytowania, debugowania i tworzenia kodu, a następnie publikowania aplikacji. Oprócz oczekiwanych funkcji, takich jak standardowy edytor i debuger, program Visual Studio dla komputerów Mac zawiera kompilatory, narzędzia do uzupełniania kodu, projektantów graficznych i kontrolę źródła, aby ułatwić proces tworzenia oprogramowania.
...
Tworzenie, otwieranie i tworzenie aplikacji będzie znanym doświadczeniem dla każdego, kto wcześniej korzystał z programu Visual Studio w systemie Windows. Ponadto program Visual Studio dla komputerów Mac wykorzystuje wiele zaawansowanych narzędzi, które sprawiają, że jego odpowiednik systemu Windows jest tak zaawansowanym IDE. Platforma kompilatora Roslyn służy do refaktoryzacji i IntelliSense. Jego system projektu i tworzenie aparatu używać MSBuild, a jego edytor źródłowy używa tego samego fundacji jako Visual Studio w systemie Windows. Używa tych samych aparatów debugera dla aplikacji Platformy Xamarin i .NET Core i tych samych projektantów dla xamarin.iOS i Xamarin.Android.
...
- ASP.NET podstawowych aplikacji sieci Web z językami C#, F#i obsługą stron Razor, JavaScript i TypeScript
...
## <a name="ide-tour"></a>Wycieczka IDE
...
Po uruchomieniu programu Visual Studio 2019 dla komputerów Mac nowi użytkownicy zobaczą okno logowania. Zaloguj się za pomocą swojego konta Microsoft, aby aktywować płatną licencję (jeśli go masz) lub połączyć się z subskrypcjami platformy Azure. Możesz nacisnąć **przycisk Zrobię to później** i zalogować się później za pomocą elementu menu Visual Studio > **Zaloguj** się:

![Logowanie się do konta Microsoft](media/ide-tour-2019-start-signin.png)

Następnie otrzymasz możliwość dostosowania IDE, wybierając preferowane skróty klawiaturowe: Visual Studio dla komputerów Mac, Visual Studio, Visual Studio Code lub Xcode:

![Wybieranie ulubionych skrótów klawiaturowych](media/ide-tour-2019-keyboard-shortcut.png)

Zalogowane użytkownicy zobaczą nowe _okno startowe,_ w którym znajduje się lista ostatnich projektów, oraz przyciski, aby otworzyć istniejący projekt lub utworzyć nowy projekt:

![Wybierz jeden z ostatnich projektów lub utwórz coś nowego](media/ide-tour-2019-start-projects.png)
...
## <a name="solution-pad"></a>Podkładka roztwórna
...
Niezależnie od tego, czy piszesz w języku C#, XAML lub Javascript, edytor kodu współużytkuje te same podstawowe składniki z programem Visual Studio Windows, z całkowicie natywnego interfejsu użytkownika.

To przynosi niektóre z następujących funkcji:
...
* Filtrowanie typu IntelliSense i "pokaż elementy importu"
* Obsługa natywnego wprowadzania tekstu
* Obsługa języka RTL/BiDi
* Roslyn 3
* Obsługa wielu karetek
* Zawijanie wierszy
* Zaktualizowany interfejs intellisense
* Ulepszone znajdowanie/zastępowanie
* Obsługa fragmentów kodu 
* Formatowanie zaznaczenia
* Żarówki inline

Aby uzyskać więcej informacji na temat korzystania z Edytora źródeł w programie Visual Studio dla komputerów Mac, zobacz dokumentację [Edytora źródeł.](/visualstudio/mac/source-editor)

Aby zawsze były widoczne w zakładkach, możesz skorzystać z ich przypinania. Dzięki temu przy każdym uruchomieniu projektu zawsze pojawi się potrzebna karta. Aby przypiąć kartę, umieść wskaźnik myszy na karcie i kliknij ikonę _pinezki:_
...
Visual Studio dla komputerów Mac ma debugery, które obsługują .NET Core, .NET Framework, Unity i Xamarin projektów. Visual Studio dla komputerów Mac używa debugera .NET Core i debugera mono soft, umożliwiając IDE do debugowania kodu zarządzanego na wszystkich platformach. Aby uzyskać dodatkowe informacje na temat debugowania, odwiedź artykuł [debugowania.](/visualstudio/mac/debugging)
